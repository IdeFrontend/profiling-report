...
- **Splitters:** **1px** `#3a3a3a` horizontal border between consecutive tracks (same as lane row dividers); no empty gap margin.
...
2. **PR-OV-002** — Each track SVG is 16px tall; consecutive tracks are separated by a 1px `#3a3a3a` horizontal splitter (no 8px gap).
...
6. **PR-OV-006** — Shared cursor x draws a continuous vertical playhead over the chart column (not gutter). Hovering anywhere in a chart column’s 16px height emits `cursor` and shows the step value tip plus a round value dot; hovering the header-row chart band emits `cursor` with a real xRatio (no tip); gutter labels do not emit `cursor`.
...
8. **PR-OV-008** — Section header is a Card-like collapsible strip; click toggles `collapsed` / `update:collapsed` and hides/shows all tracks.## Visual
