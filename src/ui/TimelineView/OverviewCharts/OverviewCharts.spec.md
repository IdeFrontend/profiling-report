...
- **Y domain:** shared **0–100** (`OVERVIEW_Y_MAX`) for all tracks — util counters are comparable; do **not** auto-scale each series to its own peak.
...
8. **PR-OV-008** — Section header is a Card-like collapsible strip; click toggles `collapsed` / `update:collapsed` and hides/shows all tracks. Click on the header **chart band** (`overview-header-track`) does **not** toggle collapse (stop propagation) — only the gutter / chevron+label strip does.
...
10. **PR-OV-010** — All tracks share a **0–100** Y domain (`OVERVIEW_Y_MAX`); a series whose peak is below 100 does not fill the band height.
