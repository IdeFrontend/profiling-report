...
**Producer tick labels.** Overview axis labels show CTEF `ts` (origin t = 0), matching tooltip/cursor and external trace tools.
...
3. **PR-OVERVIEW-003** — Leftmost tick label shows producer time when `minTime ≠ 0`.
...
- **2026-08-24** — Producer timestamp labels (origin t = 0); interop with CTEF ts.
