...
| Metric | `bar.label` (thick bars) | `barWidth` | `thresholdColor` |
|--------|--------------------------|------------|------------------|
| 时钟周期 (clockCycle) | Integer cycle count (no unit suffix) | Normalized to max lane in Card | false |
| 缓存命中率 (cacheHit) | Decimal hit rate `0.00`–`1.00` | Normalized to max lane in Card | false |
| 任务 (task) | Integer event count | Normalized to max lane in Card | false |
| 利用率 (utilization) | `NN%` | Equals util % (0–100) | true |
...
| Value fill | **利用率 / legacy:** util &lt; 0.5 → `rgba(231,67,74,0.4)` (red); util ≥ 0.5 → `rgba(255,255,255,0.08)` (gray). **Other metrics:** gray fill; when one lane is max (`relativeMax` / `barWidth === 100`) use red for that lane only; when all lanes tie, uniform gray. Never pipe/category `lane.color`. Composited over an opaque `--pr-util-track` base so the filled portion is **solid** and the hatch stops at the filled edge |
| Midline | `1px dashed rgba(255,255,255,0.1)` at `left: 50%` for **利用率**; for other metrics at `left: averageBarWidth%` when computable; above fill, under text; omit on empty slots |
