...
**Card strips.** Each Card header is a full-width opaque strip (`rgb(42, 42, 42)`, hover `rgb(50, 50, 50)`) spanning gutter + swimlane, painted above the gutter resize handle so the seam does not cut through Card rows. Header **Y** comes from `rebuildLayout(model).headers` (same walk as the canvas). Only the gutter **label** button captures pointer events (`pointer-events: auto`); the strip container and swim column stay `pointer-events: none` so pan/zoom/measure reach `SwimlaneCanvas`. Click toggles expand/collapse (`toggle-group`). Chevron + name sit in the left (gutter) column; the LaneGutter Card row is a non-interactive height spacer only.
...
2. **PR-SWIMVIEW-002** — Card strip label toggles group at header Y from layout; click emits `toggle-group`.
...
5. **PR-SWIMVIEW-005** — Card strip container is `pointer-events: none`; only `.pr-card-strip__label` is interactive.

## Changelog
- **2026-08-19** — Card header Y from `rebuildLayout`; label-only hit target; PR-SWIMVIEW-005.
