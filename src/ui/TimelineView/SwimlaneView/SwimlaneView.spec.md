...
33. **PR-SWIMVIEW-033** — A collapse tween that shrinks content while scrolled to the bottom clamps live `scrollY` / gutter `scrollTop` to the visual content height so Card strips and gutter stay aligned with the canvas.
...
## Changelog
- **2026-09-17** — PR-SWIMVIEW-033: collapse at max scroll clamps live `scrollY` so gutter/Card strips do not oppose event rows.
