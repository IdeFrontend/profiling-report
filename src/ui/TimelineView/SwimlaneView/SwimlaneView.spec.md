...
When **pinnedOverviewIds** is non-empty, a sticky **OverviewCharts** strip (`variant=strip`) renders **below** the lane pin strip and **above** the scrolling body, in overview pin order (PyPTO counter-thread pin parity). Unpinned 统计分析 tracks live at the top of the scroll body and move with `scrollY`.
