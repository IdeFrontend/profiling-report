...
**layout**, **selectedEventId**, and **dependencyMode** (`all` / `predecessors` / `successors`, default `all`). Endpoint times and content-space Y come from laid-out events (`dependencyLinks`). `EventRef` `{tid, index}` resolves through `lanesByTid` + `eventsById` (thread.events order, not duration-sorted `layout.events`). Overlay and Canvas fallback cache neighbor ids and rebuild them only when layout identity, `selectedId`, or `dependencyMode` changes. The WebGL pass applies pan/zoom/scroll as uniforms so the instance buffer is rebuilt only on selection/layout/mode change.
