...
104. **PR-CANVAS-104** — Vertical wheel eases `scrollY` toward the stacked target (not a single jump); `prefers-reduced-motion: reduce` snaps. In-flight frames still draw resting event labels, selected/hovered lifts, and collapsed-folder summary bars (same paint as a settled frame). The settle paint keeps the eased Y (does not snap to a stale parent `scrollY`). Gutter-forwarded wheel uses this same path.
...
## Changelog
- **2026-09-18** — In-flight lane-scroll frames keep resting event labels (`PR-CANVAS-104` / `PR-RENDER-056`).
