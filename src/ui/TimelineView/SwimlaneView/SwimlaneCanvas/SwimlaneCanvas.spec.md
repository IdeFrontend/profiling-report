...
Eight interaction events: **select** fires with a `SwimEvent` (or null) on click (post-4px-gate). **multi-select** fires with the `SwimEvent[]` a marquee captured, on its pointerup; a Shift+left-click toggle also commits through it, with the full post-toggle set (empty when the last event is removed). **multi-select-preview** fires with the same event list the commit will use while the marquee is live (plain rect or Shift union), and with `null` when the drag ends or is cancelled (Escape / end without commit), so the root can drive the dock without touching host `select`. **multi-select-span** fires with the live marquee's time extent while it is dragged (and `null` when it ends), so the axis can draw Δt chrome. **hover** fires on pointermove with the hovered event plus `clientX`/`clientY` for tooltip positioning. **cursor** fires with `{ time, xRatio }` for playhead placement. **pan** fires with a time-unit delta on Ctrl/Cmd+drag, Shift+wheel, or trackpad horizontal scroll. **zoom** fires with `[factor, anchorTime]` on Ctrl+wheel. **scroll-y** fires with the vertical scroll offset. **set-playhead** fires with a time value on every pointerdown (before the 4px drag gate, before hit test). The parent ProfilingReport translates all of these into viewport state changes.
...
101. **PR-CANVAS-101** — Once a marquee is live (>4px), a coverage change emits `multi-select-preview` with the same event list commit will use (plain rect, or Shift union). Unchanged coverage does not re-emit (ordered id compare, no Set). Escape, end-without-rect, and post-commit emit `null`. Commit emits `multi-select` before the clearing `null` preview.
