...
104. **PR-CANVAS-104** — Vertical wheel eases `scrollY` toward the stacked target (not a single jump); `prefers-reduced-motion: reduce` snaps. In-flight frames skip the event-label pass and clear the WebGL 2D overlay so stale fills/labels do not sit on moved event rects; labels restore when the ease settles. The settle paint keeps the eased Y (does not snap to a stale parent `scrollY`). Gutter-forwarded wheel uses this same path.
...
## Changelog
- **2026-09-16** — In-flight wheel frames `overlay.clear()` so stale overlay rects do not composite over moved GL events (`PR-CANVAS-104`).
