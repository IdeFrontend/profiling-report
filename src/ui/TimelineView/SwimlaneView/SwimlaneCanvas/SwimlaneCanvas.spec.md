...
**select** fires with a `SwimEvent` (or null) on click (post-4px-gate). **hover** fires on pointermove with the hovered event plus `clientX`/`clientY` for tooltip positioning. **cursor** fires with `{ time, xRatio }` for playhead placement. **pan** fires with a time-unit delta during drag. **zoom** fires with `[factor, anchorTime]` on Ctrl+wheel. **scroll-y** fires with the vertical scroll offset. **set-playhead** fires with a time value on every pointerdown (before the 4px drag gate, before hit test). **context-menu** fires on native right-click within a leaf lane with `{ target: SwimEvent | null, x: number, y: number, laneId: string }`; **x** and **y** are client viewport coordinates; **target** is null for an empty portion of that lane. The parent ProfilingReport translates all of these into viewport state changes.
...
74. **PR-CANVAS-074** — Native menu stays suppressed.
75. **PR-CANVAS-075** — Right-click emits resolved target.
...
| Right-click outside a leaf lane | No context-menu emit |
...
## Changelog
- **2026-09-08** — Native right-click prevents browser context menu and emits hit-tested `context-menu` payload using client viewport coordinates (`PR-CANVAS-074`/`075`).
