...
89. **PR-CANVAS-089** — Once a marquee drag crosses the 4px gate, lane hover highlight is hidden and the `cursor` event follows the pointer with `snapped: false`; the cursor is cleared when the drag ends or is cancelled. A pending press (under the gate) does not clear lane hover.
...
100. **PR-CANVAS-100** — A pending marquee press (click under the 4px gate) does not emit `lane-hover` null on `pointerdown` or `pointerup` — the gutter/header row highlight stays on the lane under the pointer for the whole click (no disappear/reappear flicker).
...
## Changelog
- **2026-09-12** — Defer lane-hover suppress until the marquee crosses the 4px gate so clicks do not flash the gutter/header highlight (`PR-CANVAS-089` / `PR-CANVAS-100`).
