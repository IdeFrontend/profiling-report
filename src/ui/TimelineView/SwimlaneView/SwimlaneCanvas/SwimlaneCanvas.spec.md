...
65. **PR-CANVAS-065** — Clicking a collapsed-group summary bar emits `toggle-group` with the folder id, clears hover (`hover` null so the summary tooltip dismisses), and emits `select` null (clears any prior selection; the summary bar is never itself selected) in both default and measure modes.
