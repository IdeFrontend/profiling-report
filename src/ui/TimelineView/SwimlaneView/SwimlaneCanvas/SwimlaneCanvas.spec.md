...
85. **PR-CANVAS-085** — While the rect is live, the events it currently covers are pushed to `setMultiSelection` so they stay bright and the rest dim; `setSelection` clears the pre-drag `selectedId` for that paint so dependency neighbors / fill lift do not keep the old single selection unmuted — except when the rect covers **exactly one** event, which demotes to single-select so its dependency curves draw; the preview is dropped on commit/cancel.
