...
**model** carries the complete `SwimlaneModel` (processes, threads, events, time bounds) or `null` when no data is loaded. **view** carries the current `SwimlaneViewWindow` (`{ startTime, endTime, scrollY }`). **selectedEventId** and **hoveredEventId** choose each block's fill from the OKLCH state palette in `laneColors.ts` (`hover` / `selected` both `L+0.33`, with `selected` also `C×1.05`); when an event is selected, every event that is neither the selection nor one of its laid-out dependency neighbors renders solid dark-gray `#2C2C2C` with label `#969696`, selection also draws a 2px white ring over that fill, and labels take their contrast from whichever fill was painted. A hovered block keeps full opacity even when another event is selected — the selection mute/dim would otherwise wash a light fill and dark label. Under the WebGL backend the fills belong to the GL pass, so `SwimlaneOverlayPainter` underpaints the lane chrome (`LANE_HOVER_FILL` when that row is hovered, else `LANE_FILL`) then repaints any non-resting block's state fill at the same dim. **multiSelectedIds** is the committed marquee selection; the renderer keeps those events bright and dims the rest. **hoveredLaneId** (optional, from parent) is the shared whole-lane hover id — gutter pointer or a sibling canvas — and calls `setHoveredLane` without re-emitting `lane-hover`. **searchQuery** drives event name filtering in the renderer. **dependencyMode** and **dependencyDepth** filter which predecessor/successor curves and undimmed neighbors are shown.

## Outputs

Eight interaction events: **select** fires with a `SwimEvent` (or null) on click (post-4px-gate). **multi-select** fires with the `SwimEvent[]` a marquee captured, on its pointerup; a Shift+left-click toggle also commits through it, with the full post-toggle set (empty when the last event is removed). **multi-select-preview** fires with the union **ids** (`string[]`) the commit will use while the marquee is live (plain rect or Shift union), and with `null` when the drag ends or is cancelled (Escape / end without commit), so the root can drive the dock count without resolving objects on every move. **multi-select-span** fires with the live marquee's time extent while it is dragged (and `null` when it ends), so the axis can draw Δt chrome. **hover** fires on pointermove with the hovered event plus `clientX`/`clientY` for tooltip positioning. **cursor** fires with `{ time, xRatio }` for playhead placement. **pan** fires with a time-unit delta on Ctrl/Cmd+drag, Shift+wheel, or trackpad horizontal scroll. **zoom** fires with `[factor, anchorTime]` on Ctrl+wheel. **scroll-y** fires with the vertical scroll offset. **set-playhead** fires with a time value on every pointerdown (before the 4px drag gate, before hit test). The parent ProfilingReport translates all of these into viewport state changes.
...
**Scroll model.** The container uses `overflow: hidden` with a synthetic scroll mechanism: a sizer div sets the total content height, and `localScrollY` tracks the actual scroll offset. The drawing surface is sized to the **visible viewport** only; lanes are scrolled via `scrollY` in the renderer. Vertical wheel eases `localScrollY` toward the stacked target (same values the gutter applies) so a notch is not a single jump; `prefers-reduced-motion: reduce` snaps.

**Pointer translation.** `pointerdown` records the starting position. `pointermove` performs hitTest (**device-pixel** coords = CSS local × `dpr`) and emits `hover` (with clientX/clientY for tooltip positioning) and `cursor` (time + xRatio + `snapped` for playhead). The CSS mouse cursor on the swim canvas is **`default`** (arrow) on empty space — not `crosshair` or `pointer`; hovering an event uses `pointer`; `measureMode` switches to `col-resize`. The swimlane cursor **always** magnetizes to the nearest in-lane event start/end within ~10px CSS (leaf or collapsed-folder summary lane under the pointer); outside that threshold it uses linear `timeAtX` (CSS). While magnetized, 2px blue bars (`measure-edge-snap`) paint on **every** visible event edge whose time exactly equals the snapped time (same projection as the committed `measure-edge-exact` marks), the `cursor` emit carries `snapped: true` (graying the full-height swim/axis line), and the magnetized event is treated as hovered (tooltip / highlight) and is selected on click even when the pointer is slightly outside the block. **Ctrl/Cmd+wheel** zooms around the stuck timestamp (magnetized event edge, or hovered measure-border time), so the edge stays at its screen X and the pointer↔edge gap is preserved. **Shift+wheel** and a two-finger horizontal trackpad scroll (dominant `deltaX`) emit `pan` in time units — dragging does not pan, it marquees. A plain wheel scrolls lanes, and a vertical scroll carrying an incidental `deltaX` still scrolls lanes. On `pointerup`, if total movement <=4px and not measuring, `hitTest` (or the magnetized event) is emitted as `select` — **except** a collapsed-group **summary bar**, which instead emits `hover` null (dismiss the summary tooltip), `toggle-group` with the folder id (expanding that grouping node), and selects the underlying leaf event(s): one leaf → `select(sourceEvent)`; two or more → `multi-select(sourceEvents)` (marquee commit path); otherwise `select` null. Summary bars are never themselves selected; their measure-mode click also expands rather than snapping the measure range.
...
**Default-mode Alt event measure.** When `measureMode` is false, **Alt+click** an event sets an ephemeral anchor (`altMeasureAnchorId`) — it does **not** emit `select` or open details, only the normal click playhead cue. The anchored event is drawn with a non-interactive white rounded border (`pr-alt-measure-anchor`, `rgba(255,255,255)`, 2px) that stays glued to the event across scroll/pan/zoom while the session is ephemeral. While **Alt** is held and the measure is **not pinned**, the target is picked by a three-step rule: **(1)** a different hovered event measures to its relation-chosen edge (`target.start` when later, `target.end` when earlier) and is highlighted with a blue rounded border (`pr-alt-measure-anchor--target`); **(2)** a magnetized event border (the ~10px edge snap) measures to that exact border, letting the user pick a specific start/end; **(3)** otherwise the target is the free cursor, drawn as a full-height blue vertical line (`pr-alt-measure__cursor-line`) on **every** shared surface (strip and body); the anchor stick and Δt arrow stay only on the surface that owns the anchor. Δt is the distance from the anchor's nearest edge to the target (`target − anchor.end` when later, `anchor.start − target` when earlier). Same-lane measurement reuses the gap-measure border sticks + `MeasureDtArrow`; cross-lane measurement draws sticks plus a vertical blue dashed connector (`pr-alt-measure__vertical`) with horizontal stubs, and the Δt label/arrow sits on the earlier event's lane. Any `deltaNs > 0` shows the overlay; both modes use inline-or-outside label fallback when the span is too narrow for inline (no minimum screen-pixel hide gate). Hovering the anchor event itself, a target inside the anchor's time span, or touching (Δt = 0) render no overlay, leaving only the anchor highlight. **Alt+click a different event pins** the measurement (`altMeasurePinned`) when `deltaNs > 0`: overlay and highlights survive Alt release; while pinned both events use the white border (no `--target` blue), and pointermove no longer retargets. Touching (Δt = 0) Alt+click does not pin. Ephemeral **Alt+click on the same anchor** is a no-op. While pinned, **Alt+click any event** sets that event as a new ephemeral anchor and clears the pin/target. Clearing ephemeral: **Alt keyup**, **pointermove with `altKey` false** (covers missed keyup after Alt+Tab/blur), **Esc**, Alt+click empty space, **collapsing a Card/folder**, or **pinning/unpinning any lane**. Clearing pinned: **Esc**, empty-canvas click, any non-Alt click, any `view` change (`startTime` / `endTime` / `scrollY`), entering `measureMode`, **collapsing a Card/folder**, or **pinning/unpinning any lane**. While the session is active, the default hover-gap measure is suppressed (including during pan capture/drag); Alt-retargeting keeps emitting the normal event `hover` (tooltip). The overlay is `pointer-events: none` and is recomputed against the current view window while visible. When a sticky pinned strip is present, pin-strip and body canvases **share** the Alt-measure session; each endpoint records which surface captured it so chrome follows the clicked instance (body click on a pinned lane stays on the body row, not the sticky duplicate). Crossing strip↔body does **not** clear the ephemeral target on `pointerleave` of either canvas. With no sticky strip, the scroll canvas uses `solo` so leave clears live preview. Cross-surface event↔event pairs draw split sticks / highlights plus a parent dashed vertical bridge that remains when either edge is outside the current time window.

**Reactivity.** A deep watcher on the viewport prop calls `renderer.setView()` and `renderer.render()` on every change. Model changes call `renderer.setModel()`. **collapsedIds** and **collapseAnim** call `setCollapsedIds` / `setCollapseAnim` (paint-only; layout identity unchanged) only after the live backend is attached — a pre-attach dummy renderer must not record the model identity, or `setModel` is skipped on the real backend and event blocks never paint. Selection/hover/`dependencyMode`/`dependencyDepth` changes trigger render only (layout unchanged; no page reload). A window-only paint (time / scroll) does not re-push `setMultiSelection` when the id-array identity is unchanged.

**Marquee multi-select.** An **unmodified drag** on the canvas marquees; Ctrl/Cmd+drag pans instead, and `measureMode` wins when it is on (that press creates a measure range instead, and never marquees). **Pending press (≤4px)** is visually a no-op: lane-row / gutter hover and the default-mode hover-gap Δt overlay stay as they were under the pointer — `pointerdown` must not clear them (that flashed the header and the gap arrow on every click). Once the pointer crosses the **4px** gate the press owns itself as a live marquee: it never selects or emits a hovered event (`hover` emits `null`, so the tooltip stays hidden), **lane-row hover and the hover-gap overlay are cleared**, and the `cursor` follows the pointer with `snapped: false` (vertical timestamp bar only). The rect appears at that gate and tracks both axes (time × lane Y), drawn as a 1px `rgba(66,133,244,0.8)` border over a `rgba(66,133,244,0.15)` fill above the measure chrome and above the Card strips / expanders (z-index 9). Below that threshold the press is still a **click**, so click-to-select is unchanged. While the rect is live, **`multi-select-span`** carries its time extent (`[min(t0,t1), max(t0,t1)]`) so the axis can draw Δt chrome; it emits `null` when the drag ends or is cancelled, leaving the committed hull to the root. While live, **`multi-select-preview`** carries union **ids** (plain rect coverage, or Shift union with the selection snapshotted at pointerdown) on every move past the gate — it does not resolve `SwimEvent` objects. Shrinking the live rect drops events that are no longer covered; live preview ids fed back as `multiSelectedIds` do not grow the union. Preview emits `null` on Escape, end-without-rect, and after a successful commit (commit fires `multi-select` first). On **pointerup** it emits **`multi-select`** with every leaf event whose drawn block intersects the rect — a Shift union resolves those ids through the layout map first, then the shared model resolver, so off-canvas / pinned-strip ids are not dropped; folder rows hold no events, so a rect crossing Card strips contributes none — and an empty rect commits `[]` (the root reads that as clear). **Escape** during the drag cancels without committing: the rect and preview clear immediately, but `marqueePressActive` stays set with `marqueeEscaped` until the leftover `pointerup` is swallowed (so that release does not select); the next plain click then selects. Move/up are bound on `window`, so `pointerleave` (or a release over Card strips) does not cancel. While the rect is live, holding the pointer in a top/bottom edge band continuously emits `scroll-y`, and in a left/right edge band emits `pan` (same 40px band / 12px-per-frame feel); the drag anchor stays fixed in content/time space so the rect stretches. If the wrap shrinks under the drag (dock preview), the edge band must not autoscroll from sliding under a stationary pointer — leave the band first to re-arm (`PR-CANVAS-113`). Committed ids arrive back as `multiSelectedIds` and reach the backend through `setMultiSelection`. While the rect is live the canvas **previews that commit**: the events it covers right now are pushed to `setMultiSelection` on every move, so they hold full opacity while everything else uses the same `eventEmphasis` state as a committed selection — the user sees which events the release will take. The preview is local (the parent's `multiSelectedIds` is untouched until commit) and is dropped on commit, Escape, and unmount, handing the dim back to the prop. Behavior details: [MultiSelectSummary](../../../MultiSelectSummary/MultiSelectSummary.spec.md).
...
45. **PR-CANVAS-045** — Alt+click an event sets an ephemeral anchor (white highlight) without emitting `select`; Alt+hover a later non-overlapping target renders the event-measure overlay with the directional Δt label and a blue target highlight.
...
55. **PR-CANVAS-055** — A hovered target event is highlighted with a blue border (`alt-measure-target`), distinct from the white anchor.
...
61. **PR-CANVAS-061** — While pinned, both anchor and target use the white border (no `pr-alt-measure-anchor--target` blue style).
...
65. **PR-CANVAS-065** — Clicking a collapsed-group summary bar emits `toggle-group` with the folder id, clears hover (`hover` null so the summary tooltip dismisses), and selects the underlying leaf event(s): exactly one → `select(sourceEvent)`; two or more → `multi-select(sourceEvents)` (same commit path as a marquee); missing sources → `select(null)`. The summary bar is never itself selected. Applies in both default and measure modes.
66. **PR-CANVAS-066** — Alt+click / Alt+hover on a collapsed-group summary bar treats it as a normal measure endpoint (anchor / retarget / pin); non-Alt click still expands (`toggle-group` + leaf/`multi-select` per PR-CANVAS-065).
67. **PR-CANVAS-067** — Ctrl/Cmd+left-drag pans the viewport (emits `pan`) instead of marqueeing.
...
80. **PR-CANVAS-080** — A marquee that misses every block commits an empty selection.
81. **PR-CANVAS-081** — Marquee suppresses tooltip and select; `pointerleave` does not cancel.
82. **PR-CANVAS-082** — Escape during the drag cancels without committing and keeps `marqueePressActive` until the leftover `pointerup`, which is swallowed via `marqueeEscaped` so the release does not select; the next plain click then selects.
...
85. **PR-CANVAS-085** — While the rect is live, the events it currently covers are pushed to `setMultiSelection` so they stay bright and the rest dim; `setSelection` clears the pre-drag `selectedId` for that paint so dependency neighbors / fill lift do not keep the old single selection unmuted; the preview is dropped on commit/cancel.
...
89. **PR-CANVAS-089** — Once a marquee drag crosses the 4px gate: lane hover is hidden, the hover-gap Δt overlay is cleared, and the `cursor` event follows the pointer with `snapped: false` (vertical timestamp bar only, plus the blue marquee rect). The cursor is cleared when the drag ends or is cancelled. A pending press (under the gate) does not clear lane hover or the hover-gap overlay and does not force an unsnapped cursor.
...
107. **PR-CANVAS-107** — Live marquee moves after the 4px gate coalesce to the latest sample on animation frame (pointerup flushes); canvas client origin is cached for the gesture so dock layout cannot force `getBoundingClientRect` mid-drag. After the gate, the canvas `pointermove` handler does not call `getBoundingClientRect`.
...
112. **PR-CANVAS-112** — While a marquee is live, holding the pointer in a top/bottom edge band emits continuous `scroll-y` toward that edge (clamped). Leaving the band, pointerup, or Escape stops autoscroll. The drag anchor stays fixed in **content** space so each scroll step stretches the rect (and remaps coverage) instead of shifting both corners with the lanes.
113. **PR-CANVAS-113** — When the wrap shrinks during a live marquee (dock preview mount), edge autoscroll does **not** arm from a band that slid under a stationary pointer — suspend until the pointer leaves the edge band (then re-enter to scroll). Live marquee also skips the resize `maxScrollY` clamp so a prior edge/post-commit autoscroll near max does not jump the timeline when the dock opens. The lane gutter uses `overflow-anchor: none`, and while `multi-select-preview` is live any gutter `scrollTop` change that is not the canvas-owned `scrollY` is rejected (browser scroll-anchoring must not bump scroll when the dock eats wrap height).
114. **PR-CANVAS-114** — While overscrolled (`localScrollY > maxScrollY`) during a live marquee, further `clampScrollY` steps may ease toward the new max (and never climb further) instead of snapping in one frame; wheel uses the same clamp.
115. **PR-CANVAS-115** — After a live marquee commits, if the wrap later shrinks such that the post-commit focus Y would fall outside the visible area (e.g. closed→preview dock grows to collapsed), the canvas emits `scroll-y` so that focus Y stays inside the wrap (near the bottom, with a small pad). The scroll **tweens** over the same ~200ms as the dock height transition (instant under `prefers-reduced-motion`). Selection-bottom focus Y includes full `LANE_HEIGHT` plus `contentTopPad` (layout `item.y` alone is short when overview pad is non-zero). No scroll when that Y remains visible.
116. **PR-CANVAS-116** — Post-commit focus Y when the latest edge-autoscroll that moved scroll was **not** up (none, or down): the **bottom of the bottommost selected lane row** in wrap scroll space — a mid-row release must not leave that row clipped.
117. **PR-CANVAS-117** — Post-commit focus Y when the latest edge-autoscroll that moved scroll was **up**: the mouse-up cursor content Y (not the selection-bottom Y).
...
- **2026-09-22** — Window-only paint skips `setMultiSelection` when the id array identity is unchanged (`PR-CANVAS-108`).
- **2026-09-18** — Live marquee coalesces post-gate moves to rAF and caches the canvas client origin (`PR-CANVAS-107`); unchanged coverage does not re-emit preview (`PR-CANVAS-101`).
...
- **2026-09-14** — Freeze slack preview `scrollY` at gesture start (`PR-ROOT-019`); soft-clamp marquee overscroll (`PR-CANVAS-113` / `PR-CANVAS-114`); selection-border focus includes `contentTopPad` (`PR-CANVAS-115`); restore live ≥2 dock throttle (`PR-ROOT-016`).
...
- **2026-09-13** — Post-commit selection-border scroll targets the full bottom selected lane row, not mid-row release Y (`PR-CANVAS-115` / `PR-CANVAS-116`).
- **2026-09-13** — Post-commit focus scroll tweens with the dock height transition (`PR-CANVAS-115`).
- **2026-09-13** — Wheel/`scroll-y` flush-paints with `localScrollY` so canvas stays locked to gutter/cards (`PR-CANVAS-118`).
- **2026-09-13** — Dock preview wrap shrink does not edge-autoscroll under a stationary pointer (`PR-CANVAS-113`).
- **2026-09-13** — Post-commit marquee scroll: latest edge-autoscroll up → cursor Y; else selection-bottom Y (`PR-CANVAS-117`).
- **2026-09-13** — After marquee commit, keep mouse-up Y visible when the dock grow shrinks the wrap (`PR-CANVAS-115`).
- **2026-09-13** — Live marquee edge autoscroll (`PR-CANVAS-112` / horizontal `PR-CANVAS-119`).
...
- **2026-09-10** — Cmd+left-drag joins Ctrl+left-drag as the combined pan gesture (`PR-CANVAS-067`).
...
- **2026-09-07** — Ctrl+left-click toggle set now seeds with the current single selection, so pick A + Ctrl+click B yields {A, B} instead of just B (`PR-CANVAS-087`).
