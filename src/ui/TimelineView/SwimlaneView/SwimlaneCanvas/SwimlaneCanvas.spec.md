...
104. **PR-CANVAS-104** — When the wrap shrinks during a live marquee (dock preview mount), edge autoscroll does **not** arm from a band that slid under a stationary pointer — suspend until the pointer leaves the edge band (then re-enter to scroll). Live marquee also skips the resize `maxScrollY` clamp so a prior edge/post-commit autoscroll near max does not jump the timeline when the dock opens.
