...
89. **PR-CANVAS-089** — From marquee `pointerdown` through the live drag, lane hover highlight stays hidden (including sub-4px canvas `pointermove`s — no non-null restore before the gate) and the `cursor` event follows the pointer with `snapped: false`; the cursor is cleared when the drag ends or is cancelled.
...
100. **PR-CANVAS-100** — A pending marquee press that ends as a click (`pointerup` under the 4px gate) restores `lane-hover` to the lane under the pointer without requiring a subsequent `pointermove`.
...
## Changelog
- **2026-09-12** — Marquee press keeps lane hover suppressed through the sub-4px gate (no flicker); click-up restores `lane-hover` without a follow-up move (`PR-CANVAS-089` tighten, `PR-CANVAS-100`).
