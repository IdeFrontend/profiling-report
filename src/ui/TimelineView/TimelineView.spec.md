...
**Narrow panels (MSTT).** No viewport breakpoint stacks overview/axis/body rows — layout stays side-by-side. Swim rows use `minmax(0, var(--pr-gutter-width)) minmax(0, 1fr)` so gutter and track shrink to fit (no horizontal scroll).
...
17. **PR-TIMELINE-017** — No viewport breakpoint stacks swim rows; swim-row columns use `minmax(0, …)` so gutter + track shrink (stable MSTT embedding).
