...
**Forwarding.** `TimelineView` forwards `context-menu` payloads from `SwimlaneView` to `ProfilingReport` without interpretation; coordinates stay client viewport. It also forwards `update:scrollY` from `SwimlaneView` so the root can dismiss an open context menu on any vertical scroll.
