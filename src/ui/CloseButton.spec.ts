...
describe('CloseButton', () => {
  it('PR-CLOSE-001: renders a stroked Γ£ò centered in a square button, not a text glyph', async () => {
    const wrapper = mount(CloseButton, { props: { label: 'Close' } });
    const btn = wrapper.get('button');

    // A typographic ├ù sits on the font's math axis, so it never optically centers.
    ...
    expect(w).toBe(h); // square box ΓåÆ the cross is symmetric about both axes
  ...
  it('PR-CLOSE-002: label drives both aria-label and title; click emits through', async () => {
    const wrapper = mount(CloseButton, { props: { label: 'σà│Θù¡' } });
    const btn = wrapper.get('button');
    expect(btn.attributes('aria-label')).toBe('σà│Θù¡');
    expect(btn.attributes('title')).toBe('σà│Θù¡');
