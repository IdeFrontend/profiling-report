...
- Topology **全屏** is a root sibling of `ReportLayout` (opaque overlay on `.pr-root`); it remounts `MemoryTopologyPanel` and does not unmount the layout. The control itself lives in the panel's zoom bar (only when the host passes `showFullscreen`), so the remounted overlay panel keeps zoom but shows no 全屏.
