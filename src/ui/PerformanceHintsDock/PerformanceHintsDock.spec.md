...
The pane is the host dock footer's **last** content branch: it renders only while the dock is open, the `performanceHints` capability carries at least one row, and neither a single selection nor a marquee multi-selection owns the dock — the aside's 性能分析 详情 trigger clears those first ([ProfilingReport](../ProfilingReport/ProfilingReport.spec.md) `PR-ROOT-019`). The pane renders inside the host's **standard event-detail dock**: the same `.pr-dock` footer that hosts `DetailPanel` / `MultiSelectSummary`, with the same `--pr-dock-h` height and chrome as the detail panel. There is no dedicated hints shell — the pane itself carries no shell chrome of its own.
