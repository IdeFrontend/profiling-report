...
8. **PR-LAYOUT-008** — While the aside grid track is tweening, ReportLayout signals `asideTrackAnimating` so the swimlane freezes its backing store (CSS-stretch) until the track settles.
...
## Changelog
- **2026-09-07** — Aside track tween signals `asideTrackAnimating` so the swimlane freezes its backing store mid-animation (`PR-LAYOUT-008`).
