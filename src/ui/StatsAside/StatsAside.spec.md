...
| 详情 | `12px` / `#e6e6e6` |
| Block select | Rendered when `PipeUtilization.csv` holds >1 `block_id` (PR-STATS-014b). Uses the shared block class `.pr-block-pill` (`tokens.css`) — the one definition also used by [`CsvFieldListPanel`](./CsvFieldListPanel/CsvFieldListPanel.spec.md), matching the 显示控制 field family: fill `#404040`, no border, radius `6px`, height `32px`, text `#ffffff` `12px`, `min-width: 72px`, `cursor: pointer`, design down-chevron (`appearance: none` — never the native arrow); `:focus` clears the ring, `:focus-visible` ring `2px #3078f0` offset `1px`; caption `block` `#b8b8b8`, gap `6px` |
...
- **2026-09-11** — Summary PIPE **block** select and the memory overlay switcher move off the `4px` `#2a2a2a` pill to the 显示控制 field-family chrome (`#404040` fill, `6px` radius, `32px`, white `12px`, design down-chevron, `:focus-visible` `2px #3078f0`) — the `.pr-block-pill` definition in `tokens.css` still backs both, so they cannot drift.
