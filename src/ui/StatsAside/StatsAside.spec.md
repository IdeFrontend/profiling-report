...
DATA-33a duration + DATA-33g bandwidth + DATA-33h compute. Card group renders when `taskDurationUs` **or** `bandwidthCards` is present (name/type alone do not open an empty grid).
...
16b. **PR-STATS-014b** — Summary PIPE block switcher (All \| PipeUtilization ids) when >1 block; scopes bars; syncs topology on pick and restores default on All; omits memory-only ids.
...
| 详情 | `12px` / `#e6e6e6` |
| Block select | Rendered when `PipeUtilization.csv` holds >1 `block_id` (PR-STATS-014b). Uses the shared block class `.pr-block-select` (`tokens.css`) — **the single write-down of this chrome**, also used by [`CsvFieldListPanel`](./CsvFieldListPanel/CsvFieldListPanel.spec.md), whose row links here instead of repeating the values. Matching the 显示控制 field family: fill `#404040`, no border, radius `6px`, height `32px`, text `#ffffff` `12px`, `min-width: 72px`, `cursor: pointer`, design down-chevron (`appearance: none` — never the native arrow; a native `<select>` cannot host a pseudo-element, so the glyph is a `10px` background SVG whose stroke hand-mirrors `--pr-select-chevron-color`, and it paints the same ~`6.5px`-wide V as the toolbar's border-triangle); `:focus` clears the ring, `:focus-visible` ring `2px #3078f0` offset `1px`; caption `block` `#b8b8b8`, gap `6px` |
...
- **2026-09-14** — The `--pr-select-chevron-size` token is dropped: it resized the block select's glyph but only the toolbar chevron's *positioning box*, so it could not do what its comment claimed. Each chevron's geometry stays local to its own drawing technique (SVG path vs rotated borders), and only the colour is shared — `--pr-select-chevron-color` is read by the toolbar, with the block select's baked data-URI acknowledged as a hand-kept duplicate. The two glyphs measure the same width (~`6.5px`) and the same vertical centre, so no visual change.
- **2026-09-11** — Summary PIPE **block** select and the memory overlay switcher move off the `4px` `#2a2a2a` pill to the 显示控制 field-family chrome (`#404040` fill, `6px` radius, `32px`, white `12px`, design down-chevron, `:focus-visible` `2px #3078f0`), and the class is renamed `.pr-block-pill` → `.pr-block-select` to match what both specs call the control. The `tokens.css` rule still backs both surfaces and is now the only place the values are written down; the chevron colour comes from `--pr-select-chevron-color`.
