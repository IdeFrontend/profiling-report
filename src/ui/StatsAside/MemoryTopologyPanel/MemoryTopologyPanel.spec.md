...
2. Overlay GB/s (or KB) labels on Buffer links from `model.edges`. Omit a label when the mapped CSV value is missing/`NA` (no `label` on the edge). GM↔L2 and L2↔cluster labels sit in the corridors **between** pillars (rotated −90°), not on the GM/L2 rects. VEC↔UB labels sit in the gap under the UB box on **both** AIV0 and AIV1, not on the cache fill. AIC L1/L0/Cube labels sit in the gap under the AIC boxes.
