...
13. **Chrome load failure (PR-MEMTOP-012):** if the `<image>` errors, warn once, suppress value overlays, and clear `aria-describedby` so hosts missing `/memory-topology.svg` do not see orphaned numbers or a dangling description id. The bar goes with it: a zoom control over a diagram that failed to load is dead chrome.
...
**In-box plate inventory (2026-09-14 audit) —** this note first claimed the export carries "10 in-box `%` plates (L2, UB/Scalar, CUBE/FixP, SIMT/SIMD/VEC)" and then retracted the claim as unreproducible. The retraction was **half right**: the count is real, but it belongs to the **sketch**, not to the export. Re-OCR of the sketch's in-box slots (crop `visual/memory-topology.png` of `v930/report-stats-scrolled`, chrome units → sketch px `(100+4u, 108+4v)`) reads **57.90%** (AIV0 Scalar), **56.06%** (AIV1 Scalar), **0.00%** (AIC Scalar, AIV0/AIV1 SIMT, CUBE, FixP) and **2.18%** (AIV0/AIV1 SIMD) — **nine** badges beside the L2 pillar plate, ten in all. The **export** ([`memory-topology.svg`](memory-topology.svg)) has a plate for **none of the nine**: its plate-sized rects pair up 1:1 with the **13 plated corridors** in the table above, and the only in-box value the panel ever paints is the L2 plate. So the sketch and the panel disagree on nine in-box badges — tracked as [UI-49](../../../../docs/context/questions/UI.md) — and the same audit run sketch-ward finds link values the export never plated, tracked as [DATA-42](../../../../docs/context/questions/DATA.md). The AIC row's blank values on the product fixture stay [DATA-41](../../../../docs/context/questions/DATA.md).
...
DATA-20 (L2 Peak, resolved), DATA-21, DATA-22, DATA-23, DATA-24, DATA-25, DATA-40 (edge value aggregation, open — interim DATA-40a), DATA-41 (AIC-row link values, open), DATA-42 (link values the sketch stacks and the export never plates, open), UI-35, UI-38, UI-48, UI-49 (Peak badges beyond L2, open), [view-models](../../../../specs/core/view-models.spec.md), [VIEW_DATA_MAPPING §11.2.6](../../../../docs/ui/VIEW_DATA_MAPPING.md).
