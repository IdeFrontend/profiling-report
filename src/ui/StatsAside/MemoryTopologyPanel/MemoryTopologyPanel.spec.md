...
16. **Zoom geometry (PR-MEMTOP-013):** the diagram sits in a fit box of the chrome's own 448:540 ratio. Zoom scales the drawn diagram inside it (the box itself is unchanged), so a zoomed diagram is panned with the scroll gesture the platform already gives — no drag handler, no transform on the panel. The box is **only scrollable past the fit**: at 100% and below the stage is at most the box, so the fitted box is not a scroll container at all and shows no scrollbar. It would otherwise be one over nothing — the box's height comes from `aspect-ratio` while the diagram's comes from its intrinsic ratio, and the two agree only to a rounding step (511 vs 511.0625 in Chrome), which is enough to paint a stray permanent bar on platforms that do not snap scrollable overflow the same way.
...
13. **PR-MEMTOP-013** — The diagram sits in a fit box with the chrome's 448:540 ratio; zoom scales the diagram inside the box rather than resizing the box. The box is scrollable **only** past the fit — at 100% and below the stage does not exceed the box, so the fitted box is not a scroll container and shows no scrollbar.
...
| Bar strip | `#313131` literal — the v930-sim export's 5% white lift over the panel; the overlay drops the strip, so the controls sit straight on the card (measured off `v930-sim/memory-topology-fullscreen`) |
| Bar control | `#b3b3b3` glyph, `#fff` on hover over `--pr-surface-hover`; `16px` glyph in a `24px` hit area; labels `缩小` / `放大` / `适应窗口` / `全屏` |
