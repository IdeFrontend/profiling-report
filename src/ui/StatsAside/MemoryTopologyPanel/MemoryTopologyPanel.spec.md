...
**In-box plate inventory (2026-09-14 audit) —** this note used to claim the export carries "10 in-box `%` plates (L2, UB/Scalar, CUBE/FixP, SIMT/SIMD/VEC)", which was the argument for reading a Peak(%) badge into every unit. That count could **not** be reproduced from `memory-topology.svg`: its plate-sized rects pair up 1:1 with the **13 plated corridors** in the table above (each drawn as a fill plus an outline), and the only in-box value the panel ever paints is the L2 plate. Whether the other units ever carried placeholder values (where a Peak badge would sit, and what its 100% reference is) is therefore tracked as [UI-49](../../../../docs/context/questions/UI.md), and the AIC row's blank values on the product fixture as [DATA-41](../../../../docs/context/questions/DATA.md).
...
DATA-20 (L2 Peak, resolved), DATA-21, DATA-22, DATA-23, DATA-24, DATA-25, DATA-40 (edge value aggregation, open — interim DATA-40a), DATA-41 (AIC-row link values, open), UI-35, UI-38, UI-48, UI-49 (Peak badges beyond L2, open), [view-models](../../../../specs/core/view-models.spec.md), [VIEW_DATA_MAPPING §11.2.6](../../../../docs/ui/VIEW_DATA_MAPPING.md).
