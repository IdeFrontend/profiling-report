...
3. Search keeps every field visible. A case-insensitive substring match paints that slice as a navy chip with light-blue text; the rest of the label and values stay unchanged. Clear (×) empties the query (`v930/search-highlight`).
...
3. **PR-CSV-003** — Search highlights matching labels.
...
| Search match | substring chip: fill `#1d283c`, text `#688aec` weight `600`, radius `3px`, pad `0 4px`; rest of label `#8e8e8e`; list is not filtered |
...
- **2026-08-24** — Search highlights matching label substrings (does not filter); focus border + clear (`v930/search-highlight`).
