...
16. **PR-ROOT-016** — Live `multi-select-preview` drives the dock (≥2 → MultiSelectSummary, 1 → DetailPanel) without host `select` / viewState commit; empty mid-drag keeps the last non-empty preview (does not mount a blank dock on empty-first); Escape mid-drag does not treat preview `multiSelected` as committed (root skips `onSelect(null)` while `marqueeLive`); canvas `preview(null)` restores the pre-drag dock; commit applies once on `multi-select`.
