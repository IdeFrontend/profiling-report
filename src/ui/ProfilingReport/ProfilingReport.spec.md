...
18. **PR-ROOT-018** — Closed→drag preview height is `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), sessionDockHeight)` where `sessionDockHeight` is the session collapsed or expanded height and slack is dead space below lane content in the closed-dock wrap; sparse content + tall viewport yields a preview taller than the minimum and ≤ session target; a viewport filled with lanes stays at the minimum. The preview is precomputed from the pre-mount wrap so the first paint does not flash full target then shrink.
