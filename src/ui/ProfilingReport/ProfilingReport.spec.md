...
16. **PR-ROOT-016** — Live `multi-select-preview` drives the dock (≥2 → MultiSelectSummary, 1 → DetailPanel; empty clears Detail/Summary) without host `select` / viewState commit; closed→drag keeps the footer hidden until coverage is non-empty then uses in-flow preview height toward the session target; already-open keeps session height and shows nothing-selected on empty mid-drag; empty commit closes; Escape mid-drag does not `onSelect(null)` while `marqueeLive`; canvas `preview(null)` restores the pre-drag dock; non-empty commit from a closed start grows to the **session** dock height (collapsed or expanded — expand survives clear→new marquee; the expander chevron shows that session state even when preview height is slack-capped; canvas then keeps the post-commit focus Y visible in the shrunken wrap — full bottom selected lane row, or cursor if latest edge-autoscroll was up). The first ≥2 preview of a new gesture applies immediately; further ≥2 remaps during the same gesture are throttled (~100ms) so edge-autoscroll remaps do not re-sort the summary every frame.
17. **PR-ROOT-017** — Single↔multi↔empty swaps fade content only (`pr-dock-content` out-in opacity) inside the persistent footer; shell height stays `--pr-dock-h` except the closed→drag preview→session-height grow on commit.
18. **PR-ROOT-018** — Closed→drag preview height is `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), sessionDockHeight)` where `DOCK_HEIGHT_MARQUEE_PREVIEW` fits the MultiSelectSummary **count header row only** (`{n} items selected` / Slices tab / close — table body clipped), `sessionDockHeight` is the session collapsed or expanded height, and slack is dead space below lane content in the closed-dock wrap (computed from the **gesture-start** `scrollY` and frozen wrap height — live edge autoscroll must not grow the preview); sparse content + tall viewport yields a preview taller than the minimum and ≤ session target; a viewport filled with lanes stays at the minimum. The preview is precomputed from the pre-mount wrap so the first paint does not flash full target then shrink.
...
## Changelog
- **2026-09-14** — `DOCK_HEIGHT_MARQUEE_PREVIEW` fits the MultiSelectSummary count header only (`PR-ROOT-018`).
...
- **2026-09-14** — Slack preview freezes gesture-start `scrollY`; live ≥2 dock remaps throttled after the first of the gesture (`PR-ROOT-016` / `PR-ROOT-018`).
- **2026-09-13** — `DOCK_HEIGHT_MARQUEE_PREVIEW` raised to fit MultiSelectSummary chrome + first table row (`PR-ROOT-018`).
- **2026-09-13** — Closed→drag preview targets session dock height (incl. expanded) and chevron follows session expand intent (`PR-ROOT-016` / `PR-ROOT-018`).
- **2026-09-13** — Closed→drag commit restores session dock height including expanded (`PR-ROOT-016`).
- **2026-09-13** — Closed→drag marquee preview height uses slack below lanes up to the session target (`PR-ROOT-018`).
