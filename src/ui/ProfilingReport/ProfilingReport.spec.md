...
**Live marquee dock preview.** While the canvas emits `multi-select-preview` (after the 4px gate), the root drives the dock UI from that list without calling `setMultiSelection` / host `select` on every move: **≥2** → MultiSelectSummary, **1** → DetailPanel, **empty** → clear Detail/Summary. **Gesture liveness** (`marqueeLive`) arms on any post-gate preview including `[]` so Escape is gated. Footer mount: when the dock was **already open** at drag start, empty coverage keeps the shell mounted with a nothing-selected message; when it was **closed**, the footer stays hidden until coverage is non-empty (no empty-state flash). Closed→drag uses a **preview height** of `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), DOCK_HEIGHT_COLLAPSED)` — dead space below lane content in the closed-dock wrap grows the preview up to the post-commit collapsed height; the height is **precomputed before the footer mounts** (frozen closed wrap) so the first paint is already correct and does not flash collapsed then shrink. An **already-open** dock keeps its session height. On commit (`multi-select`) the existing empty / 1 / ≥2 rules apply once — empty closes; non-empty from a closed start grows to collapsed height when the preview was still short. After that grow, the canvas keeps the mouse-up lane Y visible in the shrunken wrap. Escape mid-drag: while `marqueeLive`, root does not `onSelect(null)`; the canvas cancels and emits `preview(null)`, which restores the pre-drag dock snapshot — no host `select`. Mode switches fade **content only** (`pr-dock-content` out-in, opacity ~180ms) inside the persistent `<footer class="pr-dock">`. Outer `pr-dock` Transition is only for true none↔some (open/close). **Enter** height-tweens from `0` so the timeline shrinks with the visible panel (no empty flex slot / black hole). **Leave** takes the dock `position: absolute` and slides it away so flex space frees immediately while the panel is still on screen. The root also owns the Δt span the axis draws for a multi-selection — the live marquee extent during the drag; the canvas clears it on commit so the axis Δt disappears when the rect commits. Both docks share the one session-only `dockHeight` inside that persistent footer.
...
16. **PR-ROOT-016** — Live `multi-select-preview` drives the dock (≥2 → MultiSelectSummary, 1 → DetailPanel; empty clears Detail/Summary) without host `select` / viewState commit; closed→drag keeps the footer hidden until coverage is non-empty then uses in-flow preview height; already-open keeps session height and shows nothing-selected on empty mid-drag; empty commit closes; Escape mid-drag does not `onSelect(null)` while `marqueeLive`; canvas `preview(null)` restores the pre-drag dock; non-empty commit from a closed start grows to collapsed height when the preview was still short (canvas then keeps the post-commit focus Y visible in the shrunken wrap — selection bottom, or cursor if latest edge-autoscroll was up).
17. **PR-ROOT-017** — Single↔multi↔empty swaps fade content only (`pr-dock-content` out-in opacity) inside the persistent footer; shell height stays `--pr-dock-h` except the closed→drag preview→collapsed grow on commit.
18. **PR-ROOT-018** — Closed→drag preview height is `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), DOCK_HEIGHT_COLLAPSED)` where slack is dead space below lane content in the closed-dock wrap; sparse content + tall viewport yields a preview taller than the minimum and ≤ collapsed; a viewport filled with lanes stays at the minimum. The preview is precomputed from the pre-mount wrap so the first paint does not flash collapsed height then shrink.
...
- **2026-09-13** — Live marquee uses in-flow preview height from a closed dock; empty mid-drag shows nothing-selected (no stale rows); first ≥2 of a new gesture applies immediately over a committed multi (PR-ROOT-016).

=======
- **2026-09-13** — Closed→drag marquee preview height uses slack below lanes up to collapsed (`PR-ROOT-018`).
- **2026-09-13** — Live marquee uses in-flow preview height from a closed dock; empty mid-drag shows nothing-selected (PR-ROOT-016).
>>>>>>> 6689e2ec (fix: grow marquee preview dock into slack below lanes)
