...
- **2026-09-23** — 性能分析 (performance analysis) trigger now clears the selection / marquee before opening the hints pane, and one `hintsDockShown` computed drives both the pane and the `pr-dock--hints` chrome — the trigger used to look dead while an event was selected and the shell kept hints chrome over Detail/Summary content (`PR-ROOT-019`).
