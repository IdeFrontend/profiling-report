...
**Live marquee dock preview.** While the canvas emits `multi-select-preview` (after the 4px gate), the root drives the dock UI from that payload without calling `setMultiSelection` / host `select` on every move: production canvas emits union **ids**; tests may still emit `SwimEvent[]`. **≥2** → MultiSelectSummary, **1** → DetailPanel, **empty** → clear Detail/Summary. While `marqueeLive`, a ≥2 summary keeps its table mounted, non-interactive, and **dimmed** — the header count updates from the id count immediately (`livePreview` / `liveCount`) without assigning a `SwimEvent[]`, and the stale table stays until the marquee has been still for `MULTI_SELECT_SETTLE_MS` (200ms) or the commit arrives, at which point the root resolves the ids and recalculates the table. The table is non-interactive for the whole live gesture (not just the stale window), so a name click cannot desync the dock from the in-flight marquee; a settle that was already queued when the commit lands is ignored (`marqueeLive` is false by then), so it cannot wipe the committed selection. **Gesture liveness** (`marqueeLive`) arms on any post-gate preview including `[]` so Escape is gated. Footer mount: when the dock was **already open** at drag start, empty coverage keeps the shell mounted with a nothing-selected message; when it was **closed**, the footer stays hidden until coverage is non-empty (no empty-state flash). Closed→drag uses a **preview height** of `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), sessionDockHeight)` — dead space below lane content grows the preview toward the **session** target (collapsed or expanded); when slack caps below that target the expander chevron still shows the session expand state. The height is **precomputed before the footer mounts** (frozen closed wrap) so the first paint is already correct and does not flash full target then shrink. Slack uses the **gesture-start** `scrollY` so edge autoscroll does not grow the preview. An **already-open** dock keeps its session height. On commit (`multi-select`) the existing empty / 1 / ≥2 rules apply once — empty closes; non-empty from a closed start grows to the **session** dock height (collapsed or expanded — expand survives clear→new marquee). After that grow, the canvas minimally scrolls so the selection stays visible with margins **only when the grow would clip it** (already-open / selection-above-dock stay put; see `PR-ROOT-020` / `PR-CANVAS-115`). Escape mid-drag: while `marqueeLive`, root does not `onSelect(null)`; the canvas cancels and emits `preview(null)`, which restores the pre-drag dock snapshot — no host `select`. Mode switches fade **content only** (`pr-dock-content` out-in, opacity ~180ms) inside the persistent `<footer class="pr-dock">`; the shell height stays `--pr-dock-h` except the closed→drag preview→session-height grow on commit (no height animation on single↔multi↔empty swap). Outer `pr-dock` Transition is only for true none↔some (open/close). **Enter** height-tweens from `0` so the timeline shrinks with the visible panel (no empty flex slot / black hole). **Leave** takes the dock `position: absolute` and slides it away so flex space frees immediately while the panel is still on screen. The root also owns the Δt span the axis draws for a multi-selection — the live marquee extent during the drag; the canvas clears it on commit so the axis Δt disappears when the rect commits. Both docks share the one session-only `dockHeight` inside that persistent footer.

**性能分析 hints pane.** The M4 [performance-hints dock](../PerformanceHintsDock/PerformanceHintsDock.spec.md) is the dock content chain's **last** branch, so `hintsDockOpen` (set by the StatsAside 性能分析 trigger) plus the `performanceHints` capability only buys the slot while no single selection or marquee multi-selection is up. The trigger clears what would swallow the pane before it opens: a live marquee through `clearMarqueeLive()` (dock snapshot + marquee flags + preview throttle), then the committed selection through `onSelect(null)` — the same path the canvas uses, which clears `selected` / `multiSelected` / the Δt span / the `viewState` ids and emits the host `select(null)`. One computed, `hintsDockShown`, mirrors that branch exactly and drives both the pane and the shell's `pr-dock--hints` chrome, so the footer never wears hints chrome while DetailPanel / MultiSelectSummary is its content.
...
17. **PR-ROOT-017** — Single↔multi↔empty swaps fade content only (`pr-dock-content` out-in opacity) inside the persistent footer; shell height stays `--pr-dock-h` (no height animation on mode swap) except the closed→drag preview→session-height grow on commit.
...
## Changelog
- **2026-09-24** — Live-marquee Behavior notes post-commit ensure only when closed→session grow would clip (`PR-ROOT-020`).
- **2026-09-24** — Post-commit canvas ensure scroll only when closed→session dock grow would clip the focus Y (`PR-ROOT-020` / `PR-CANVAS-115`).
...
- **2026-09-14** — `DOCK_HEIGHT_MARQUEE_PREVIEW` fits the MultiSelectSummary count header only (`PR-ROOT-020`).
...
- **2026-09-14** — Slack preview freezes gesture-start `scrollY` (`PR-ROOT-020`).
- **2026-09-13** — `DOCK_HEIGHT_MARQUEE_PREVIEW` raised to fit MultiSelectSummary chrome + first table row (`PR-ROOT-020`).
- **2026-09-13** — Closed→drag preview targets session dock height (incl. expanded) and chevron follows session expand intent (`PR-ROOT-016` / `PR-ROOT-020`).
- **2026-09-13** — Closed→drag commit restores session dock height including expanded (`PR-ROOT-016` / `PR-ROOT-020`).
- **2026-09-13** — Closed→drag marquee preview height uses slack below lanes up to the session target (`PR-ROOT-020`).
- **2026-09-13** — Live marquee uses in-flow preview height from a closed dock; empty mid-drag shows nothing-selected (no stale rows); first ≥2 of a new gesture applies immediately over a committed multi (PR-ROOT-016 / PR-ROOT-020).
