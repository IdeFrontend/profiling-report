...
16. **PR-ROOT-016** — Live `multi-select-preview` drives the dock (≥2 → MultiSelectSummary, 1 → DetailPanel; empty clears Detail/Summary) without host `select` / viewState commit; ids-only ≥2 mounts the header from the id count without assigning events, keeps the stale table dimmed while the marquee is growing, and recalculates the table once the marquee has been still for 200ms; closed→drag keeps the footer hidden until coverage is non-empty; already-open keeps session height and shows nothing-selected on empty mid-drag; empty commit closes; Escape mid-drag does not `onSelect(null)` while `marqueeLive`; canvas `preview(null)` restores the pre-drag dock.
...
## Changelog
- **2026-09-23** — A growing live marquee dims the stale table and recalculates it after a 200ms settle (or on commit) instead of hiding it (`PR-ROOT-016`).
