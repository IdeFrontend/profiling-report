...
4. **PR-ROOT-004** — Auto-loaded sources apply the adapter's capabilities (`hardwareDetails`, `memoryDiagram`, `dependencies`); `roofline` is never derived, only honoured when the host passes it; the prop overrides the derived set wholesale; host-managed models and a removed `source` publish none and clear operator state (no stale OP selector).
