...
**dismiss** reports closure after click-outside, Escape, an item activation, viewport scroll, or any change to **scrollY** (gutter wheel, canvas wheel, trackpad pan, W/S/A/D, zoom-to-fit, collapse, etc.).

## Behavior

### Menu contents

For an event target, the menu orders available commands by scope: event-scope **重置缩放** (Reset zoom), **在事件视图中显示** (Show in event view); then lane-scope **置顶行** / **取消置顶行** (Pin row / Unpin row, Alt+P). A separator divides non-empty event and lane groups. A lane-header or empty portion of a leaf lane shows only lane-scope commands, with no separator.
...
The menu closes on click outside, Escape, item activation, viewport scroll, and every forwarded **update:scrollY** change (gutter, main or pinned canvas, Card strip, or parent update). Arrow Up and Arrow Down move active-item focus; Enter activates it. Alt+P activates Pin row only while the menu is open and Pin row is available; otherwise the chord is left untouched.
...
8. **PR-CTXMENU-008** — Dismisses on outside, Escape, viewport scroll, or scrollY change.
...
| State | Behavior |
|---|---|
| `target` is `null` | Show lane-scope commands only; if no lane-scope command is available, render no menu. |
...
## Changelog
- **2026-09-14** — Empty command sets render no menu; viewport resize re-clamps, viewport scroll dismisses, and Pin row uses Alt+P only when available (`PR-CTXMENU-007`, `PR-CTXMENU-008`, `PR-CTXMENU-010`, `PR-CTXMENU-014`).
