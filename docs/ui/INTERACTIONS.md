...
- Axis **cursor timestamp** lifts above the viewport time axis when the playhead is inside the measured range (or covers an outside Δt label), with a short animated transition; otherwise it stays in-track.
