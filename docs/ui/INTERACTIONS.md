...
- **Default-mode Alt event measure:** **Alt+click** an event to set an ephemeral anchor. This **does not** select the event, open details, or change any other view — it only moves the playhead (the normal click cue). The anchored event is marked with a non-interactive pink rounded border highlight (`rgba(255,180,196)`, 2px) for the whole time the anchor is set. While **Alt remains held**, hover another event to preview Δt between non-overlapping time ranges — reusing the gap-measure arrow/sticks on the same lane, or sticks plus a vertical blue dashed connector across lanes; the Δt label/arrow sits on the earlier event's lane, with the same inline-or-outside label fallback as the measure arrow (same-lane stays inline-or-nothing). Hovering the anchor event itself shows only the highlight (no Δt). If the anchor and target overlap in time, only the anchor highlight is shown. Touching events (Δt = 0) show nothing. Δt is `target.start − anchor.end` when the target is later, otherwise `anchor.start − target.end`. While a session is active, the default hover-gap measure is suppressed. Clearing: **Alt keyup**, **Esc**, Alt+click empty space, or Alt+click the same anchor again (toggle). Hidden in measure mode. Does not pin after Alt release.
