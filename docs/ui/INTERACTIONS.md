...
- PIPE bars: **All** (default) = mean aggregates ([DATA-28](../context/decisions/DATA.md)); one block selector (All \| 0 \| 1 \| 2 …, default **All**) scopes every widget ([DATA-19](../context/decisions/DATA.md) / [DATA-29](../context/decisions/DATA.md)). Measure range does not change them.
- Detail / memory lists are **block-scoped** via the same block switcher ([DATA-19](../context/decisions/DATA.md) / [DATA-29](../context/decisions/DATA.md)); topology labels use the same `selectedBlockId` (synced when the summary block control picks an id; **All** restores the default topology block).
