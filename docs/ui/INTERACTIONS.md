...
- Mode switches (single↔multi, including live preview) fade **dock content only** (`pr-dock-content` out-in opacity); the footer shell height stays stable.
...
=======
- Stacked 报告统计 (M2): summary cards (duration + compute/util `N/A` placeholders + I/O BW), roofline (only with the `roofline` capability), PIPE, topology — no mode-tab switcher. PIPE **详情** opens compute CSV overlay; topology **详情** and stacked-diagram **right-click** (UI-35) open memory CSV overlay; the topology diagram's own bar carries **缩小** / `%` / **放大** / **适应窗口** (discrete `50…400 %` ladder, 100% = fitted; PR-MEMTOP-015) and **全屏** (fit-window icon), which covers `.pr-root` with the current diagram and a Back control (not the browser Fullscreen API); overlay right-click does not open the memory CSV overlay; Escape closes the overlay (after measure-clear); W/S/A/D do not pan/zoom the covered timeline. Aside overlay back returns to the stack.
- Memory diagrams (stacked and overlay) each own their zoom: **缩小** / **放大** step a discrete `50, 75, 100, 125, 150, 200, 300, 400 %` ladder, the readout shows the stop, **适应窗口** returns to 100% and to the diagram's scroll origin, and the ends disable rather than no-op. 100% means *fitted to its box*, so the same readout is correct in the stacked aside and in the fullscreen overlay, and the two instances keep separate zoom. At or below the fit the diagram box is **not** a scroll container — it has no scrollbar, and the wheel over it scrolls the aside as usual; panning appears only once a zoom past the fit makes the diagram larger than its box, the pan covers exactly the diagram's own overflow (in the wide fullscreen box as well as the aside, so it never travels through empty stage), and stepping back down to a fitted stop returns to the origin the way **适应窗口** does. Each step of the ladder **keeps the middle** (PR-MEMTOP-018): the part of the drawing under the middle of the box before the step is under it after, so a fitted diagram grows about its own middle and a panned one grows about what is on screen, instead of the offset staying at the top-left corner and the drawing sliding away. Past the fit the drawing is **also pannable by dragging it** (PR-MEMTOP-017): a primary-button drag moves the box's own scroll 1:1 with the pointer, `grab` → `grabbing`, so the bars are one of two ways to pan rather than the only one — a press with a non-primary button, or on a **classic** scrollbar, or with a finger (touch stays with the platform's own scroll), is left to the platform (an overlay bar is drawn over the drawing and pans with it). Right-click is the diagram's gesture: on the bar it neither opens the memory CSV overlay nor leaves the browser menu. This scales the diagram only — it is **not** the timeline's time-axis zoom and it does not touch `timeWindow` ([PR-MEMTOP-015](../../src/ui/StatsAside/MemoryTopologyPanel/MemoryTopologyPanel.spec.md)).
- PIPE bars: **All** (default) = the `summary.jsonl` `PipeUtilization` aggregate; a picked id = that block's `PipeUtilization.csv` row. One block selector (All \| 0 \| 1 \| 2 …, default **All**) scopes every CSV-backed widget — PIPE, topology, BW, compute, roofline ([DATA-19](../context/decisions/DATA.md) / [DATA-28](../context/decisions/DATA.md) / [DATA-29](../context/decisions/DATA.md)). Measure range does not change them. MIX ops: Cube\|Vector toggle. Emulate multi-core hist: AIC\|AIV0\|AIV1 toggle ([UI-54](../context/decisions/UI.md)).
>>>>>>> a51ba1bc (feat: AIC|AIV0|AIV1 PIPE toggle for emulate occupancy)
