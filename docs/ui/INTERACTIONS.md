...
- Click unpinned pushpin → parent appends lane id to **pinnedLaneIds**; click pinned → remove. Context-menu **Pin row** (Shift+P while the menu is open) toggles the same **pinnedLaneIds** — one pin state, two affordances.
...
- Right-click lane or event → menu (e.g. **Reset zoom** at any timeline point while zoomed in, **Pin row** + Shift+P while open, copy name, reveal in details). **Pin row** writes the same **pinnedLaneIds** as the gutter pushpin; gutter icon is the primary affordance in this pass.
