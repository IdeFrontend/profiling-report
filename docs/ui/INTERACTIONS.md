...
- **Post-commit visibility:** when release grows a closed dock from preview to collapsed height, the swimlane may shrink under the selection — after the dock settles, scroll so the focus Y stays visible. If the latest edge-autoscroll during the drag was **up**, keep the mouse-up cursor Y visible; otherwise (no edge scroll, or latest was **down**) keep the **full bottom selected lane row** visible (wrap space, including `contentTopPad`).
