...
- **Post-commit visibility:** only when release grows a **closed** dock from preview to session height (wrap shorter than at drag start) **and** the focus Y would be clipped by that shrink — after the dock settles, scroll so the focus Y stays visible. If the selection (focus Y) remains fully above the final dock / inside the settled wrap, do **not** adjust scroll. An already-open dock at unchanged height never triggers release scroll. If the latest edge-autoscroll during the drag was **up**, focus is the mouse-up cursor Y; otherwise (no edge scroll, or latest was **down**) focus is the **full bottom selected lane row** (wrap space, including `contentTopPad`).
