...
- **Post-commit visibility:** when release grows a closed dock from preview to collapsed height, the swimlane shrinks under the pointer — scroll so the mouse-up lane Y stays inside the visible wrap after the dock height settles.
