...
- **Default-mode Alt event measure:** **Alt+click** an event to set an ephemeral anchor (does not select the event or open details). While **Alt remains held**, hover another event to preview Δt between non-overlapping time ranges — reusing the gap-measure arrow/sticks on the same lane, or sticks plus a vertical blue dashed connector across lanes. If the anchor and target overlap in time, only the anchor highlight is shown. Δt is `target.start − anchor.end` when the target is later, otherwise `anchor.start − target.end`. Clearing: **Alt keyup**, **Esc**, Alt+click empty space, or Alt+click the same anchor again (toggle). Hidden in measure mode. Does not pin after Alt release.
