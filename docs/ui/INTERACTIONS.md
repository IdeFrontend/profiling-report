...
- Axis **cursor timestamp** lifts above the viewport time axis when its pill overlaps the measured range (including when the playhead is just outside a border but the pill still crosses it) or covers an outside / offscreen Δt label, with a short animated transition; otherwise it stays in-track.
- **Clipped / offscreen edges:** do not draw a bar or arrowhead for a measure edge that lies outside the current view (avoids a false “selection ends at the screen edge” cue). When the whole range is off-screen, the time axis keeps a one-sided near-edge cue (cue bar + pointing chevron + Δt); swimlane fade/borders stay hidden.
- **Edge resize:** left/right measure bars (axis blue + swimlane gray) are draggable when that true edge is in view. Hover uses `col-resize` and thickens the stem to 2px; drag moves that edge with a ~1px min span and clamps into the current view. Empty-axis / empty-swimlane drag still creates a new range. Hovering a measure edge **sticks** the cursor timestamp to that border (does not hide it); the pill lifts above when it overlaps the bar. Offscreen cue bars are not resizable.
