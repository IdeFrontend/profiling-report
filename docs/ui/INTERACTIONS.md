...
- PIPE bars default to mean aggregates ([DATA-33b](../context/decisions/interim/DATA.md)); when >1 block, summary **block** control (All \| id) scopes PIPE (PR-STATS-014b). Measure range does not change them.
- Detail / memory lists are **block-scoped** via block switcher ([DATA-33c](../context/decisions/interim/DATA.md)); topology labels use the same `selectedBlockId` (synced when the summary block control picks an id).
