...
- Compute details overlay: under **All**, `SummaryCategoryList` (`summary.jsonl` categories); with a block picked, CSV tabs PipeUtilization | PipeUtilizationHist ([UI-55](../context/decisions/UI.md)) | ArithmeticUtilization | ResourceConflictRatio. Both surfaces: type filter hides non-matching labels and paints the matching substring as a navy chip ([UI-43](../context/decisions/UI.md)).
