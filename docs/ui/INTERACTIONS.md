...
- **Live marquee (>4px):** hide lane-row hover and the hover-gap overlay; show only the blue marquee rect plus the unsnapped vertical cursor / timestamp bar that follows the pointer. Axis Δt chrome follows the marquee's time extent. The bottom dock updates live from coverage: **1 event → DetailPanel**, **≥2 → MultiSelectSummary**. If the dock was **already open**, empty coverage shows a nothing-selected message (no stale rows). If the dock was **closed** at drag start, it stays closed until coverage is non-empty, then opens in-flow at a **preview height** of `min(max(DOCK_HEIGHT_MARQUEE_PREVIEW, slackBelowLanes), sessionDockHeight)` — growing into dead space below lane content toward the session collapsed/expanded target; when slack caps the painted height, the expander chevron still shows that session expand state. Slack uses the **gesture-start** `scrollY` (frozen with the closed wrap) so edge autoscroll does not grow the preview and re-suspend. An already-open dock keeps its height. Host selection / viewState commit only on release (preview grows to the session dock height, or empty closes).
- **Live multi-select table:** while the marquee is still growing (less than 200ms since the last event joined the selection), the summary table stays mounted but **dimmed** (stale rows, non-interactive) and the count header updates live; the table is **recalculated** once the selection finishes or the marquee has been still for 200ms.
- **Edge autoscroll:** while the marquee is live, holding the pointer in a band near the **top or bottom** of the visible swimlane scrolls lanes continuously (`scroll-y`), and near the **left or right** pans the time window (`pan`) — same band width and per-frame CSS-px step — so the rect can cover off-screen events. The drag **anchor stays fixed in content/time space** so the selection stretches with scroll/pan instead of shifting as a rigid box. Dock preview shrinking the wrap must not start edge autoscroll by sliding the band under a stationary pointer — leave the band to re-arm. Dock mount must also not change `scrollY` via gutter scroll-anchoring while the preview is live. After a dock-shrink overscroll (`scrollY > maxScrollY`), further steps may ease toward the new max without jumping the timeline to it in one frame.
- **Post-commit visibility:** only when release grows a **closed** dock from preview to session height (wrap shorter than at drag start) **and** the selection would be clipped by that shrink — after the dock settles, scroll by the **minimum** needed so the selected lane range stays visible with a small margin above and below (delta capped at the preview→session wrap shrink). If the selection already fits above the final dock, do **not** adjust scroll. An already-open dock at unchanged height never triggers release scroll. If the latest edge-autoscroll during the drag was **up**, the reveal target is the mouse-up cursor Y; otherwise it is the selected lane range (wrap space, including `contentTopPad`).

- **Measure mode wins:** while the caliper is on, an unmodified drag creates / resizes `measureRange` and never marquees.
- Marquee commit replaces any single selection (and vice versa); an empty rect clears both. A rect that covers **exactly one** event is treated as a normal single selection (DetailPanel), not the multi-select summary. Escape cancels mid-drag — including an empty-only live rect over a committed multi — restores the pre-drag dock and does not emit host `select`; Escape also clears a committed selection when no live marquee is active.
- Mode switches (single↔multi↔empty, including live preview) fade **dock content only** (`pr-dock-content` out-in opacity); the footer shell height stays stable except the closed→drag preview→collapsed grow on commit.
...
- Aside **close** (report shell only) clears `asideVisible` (equivalent to toolbar stats toggle off). Compute / memory / hardware overlay headers omit × so drill-in cannot dismiss the whole aside — back returns to the stack (PR-STATS-006b). See [StatsAside.spec.md](../../src/ui/StatsAside/StatsAside.spec.md).
- **更多** / More opens interim `HardwareDetailsPanel` (DATA-34a) when data exists and emits `open-hardware-details`.
- Stacked 报告统计 (M2): summary cards (duration + compute/util `N/A` placeholders + I/O BW), roofline (only with the `roofline` capability), PIPE, topology — no mode-tab switcher. PIPE **详情** opens compute CSV overlay; topology **详情** and stacked-diagram **right-click** (UI-35) open memory CSV overlay; the topology diagram's own bar carries **缩小** / `%` / **放大** / **适应窗口** (discrete 1.5× ladder from 100% clamped to `50…500 %`, 100% = fitted; PR-MEMTOP-015) and **全屏** (fit-window icon), which covers `.pr-root` with the current diagram and a Back control (not the browser Fullscreen API); overlay right-click does not open the memory CSV overlay; Escape closes the overlay (after measure-clear); W/S/A/D do not pan/zoom the covered timeline. Aside overlay headers show back and omit close; back returns to the stack. Compute / memory / hardware drill-in overlays fade opacity-only over the full aside (back + title with the tables; `pr-aside-detail` / PR-STATS-039).
- Memory diagrams (stacked and overlay) each own their zoom: **缩小** / **放大** step a discrete **1.5×** geometric ladder (`50, ≈67, 100, 150, 225, ≈338, 500 %`), the readout shows the stop, **适应窗口** returns to 100% and to the diagram's scroll origin, and the ends disable rather than no-op. 100% means *fitted to its box*, so the same readout is correct in the stacked aside and in the fullscreen overlay, and the two instances keep separate zoom. At or below the fit the diagram box is **not** a scroll container — it has no scrollbar, and the wheel over it scrolls the aside as usual; panning appears only once a zoom past the fit makes the diagram larger than its box, the pan covers exactly the diagram's own overflow (in the wide fullscreen box as well as the aside, so it never travels through empty stage), and scrollbar visibility does **not** resize the drawing (stage sized from the fit frame, not the scrollport client area; PR-MEMTOP-013b). Stepping back down to a fitted stop returns to the origin the way **适应窗口** does. Each step of the ladder **keeps the middle** (PR-MEMTOP-018): the part of the drawing under the middle of the box before the step is under it after, so a fitted diagram grows about its own middle and a panned one grows about what is on screen, instead of the offset staying at the top-left corner and the drawing sliding away. Past the fit the drawing is **also pannable by dragging it** (PR-MEMTOP-017): a primary-button drag moves the box's own scroll 1:1 with the pointer, `grab` → `grabbing`, so the bars are one of two ways to pan rather than the only one — a press with a non-primary button, or on a **classic** scrollbar, or with a finger (touch stays with the platform's own scroll), is left to the platform (an overlay bar is drawn over the drawing and pans with it). The **fullscreen overlay** also turns on **`wheelGestures`** so plain vertical wheel can zoom at fit (PR-MEMTOP-020); **Ctrl/Cmd+wheel and trackpad pinch** apply a continuous scale on the stacked aside too (bar **+**/**−** keep the discrete ladder), while unmodified vertical wheel there still scrolls the report. Two-finger horizontal / Shift+wheel pan when past the fit. The stacked aside leaves `wheelGestures` off. Right-click is the diagram's gesture: on the bar it neither opens the memory CSV overlay nor leaves the browser menu. This scales the diagram only — it is **not** the timeline's time-axis zoom and it does not touch `timeWindow` ([PR-MEMTOP-015](../../src/ui/StatsAside/MemoryTopologyPanel/MemoryTopologyPanel.spec.md)).
- PIPE bars: **All** (default) = the `summary.jsonl` `PipeUtilization` aggregate; a picked id = that block's `PipeUtilization.csv` row. One block selector (All \| 0 \| 1 \| 2 …, default **All**) scopes every CSV-backed widget — PIPE, topology, BW, compute, roofline ([DATA-19](../context/decisions/DATA.md) / [DATA-28](../context/decisions/DATA.md) / [DATA-29](../context/decisions/DATA.md)). Measure range does not change them. MIX ops: Cube\|Vector toggle. Emulate multi-core hist: Cube\|Vector 0\|Vector 1 toggle ([UI-54](../context/decisions/UI.md)).
...
- PIPE side toggle: Cube | Vector for MIX ops; Cube | Vector 0 | Vector 1 for emulate multi-core hist ([UI-54](../context/decisions/UI.md)).
...
- Compute details overlay: under **All**, `SummaryCategoryList` (`summary.jsonl` categories); with a block picked, CSV tabs PipeUtilization **or** Pipe Utilization ([UI-55](../context/decisions/UI.md): hist preferred; omit the wide CSV tab when hist projects) | ArithmeticUtilization | ResourceConflictRatio. Both surfaces: type filter hides non-matching labels and paints the matching substring as a navy chip ([UI-43](../context/decisions/UI.md)).
