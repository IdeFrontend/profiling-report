...
- **Live marquee (>4px):** hide lane-row hover and the hover-gap overlay; show only the blue marquee rect plus the unsnapped vertical cursor / timestamp bar that follows the pointer. Axis Δt chrome follows the marquee's time extent. The bottom dock updates live from coverage: **1 event → DetailPanel**, **≥2 → MultiSelectSummary**. If the dock was **already open**, empty coverage shows a nothing-selected message (no stale rows). If the dock was **closed** at drag start, it stays closed until coverage is non-empty, then opens in-flow at a **preview height** of `min(max(56, slackBelowLanes), collapsed)` — growing into dead space below lane content up to the post-commit collapsed height so a sparse timeline can show a near-full dock without shrinking the swimlane under the pointer; an already-open dock keeps its height. Host selection / viewState commit only on release (preview grows to collapsed if still short, or empty closes).
- **Edge autoscroll:** while the marquee is live, holding the pointer in a band near the top or bottom of the visible swimlane scrolls lanes continuously so the rect can cover off-screen events.
