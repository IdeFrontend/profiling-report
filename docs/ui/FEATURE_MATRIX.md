...
| Alt event measure (Alt+click + hover) | M | Default-mode Alt+click anchor + Alt+hover Δt between non-overlapping events (same-lane reuse or cross-lane dashed connector); ephemeral, does not change selection or window |
