...
| Compute-load detail tabs | M1 | `PipeUtilization` \| `PipeUtilizationHist` ([UI-55](../context/decisions/UI.md): sparse `aic_`/`aiv0_`/`aiv1_` `*_ratio`) \| `ArithmeticUtilization` \| `ResourceConflictRatio` (#3); searchable field lists (filter + highlight, [UI-43](../context/decisions/UI.md)); hist preferred + omit PipeUtilization when hist present |
