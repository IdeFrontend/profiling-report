...
| Cube / Vector overview charts | M | From `Sampling.json` `ph:C` — all counters present ([DATA-39](../context/decisions/DATA.md)); **hide** if empty ([DATA-32](../context/decisions/DATA.md)); per-track pin → sticky strip below pinned lanes (PyPTO) |
