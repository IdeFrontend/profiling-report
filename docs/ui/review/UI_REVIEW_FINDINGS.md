...
| AC-19 | Partial | .1 landed with the pin slice (#51); .2 was never implemented and is fixed here (label lifts to `#fff`). The design pins rows that carry chevrons, which the merged slice deliberately excludes — see below |
