...
<<<<<<< HEAD
**Interim:** Card-header dropdown has **exactly two** items: **利用率 / Utilization** (unchanged — event coverage per [utilization.spec.md](../../../../specs/core/utilization.spec.md); Product 耗时占比) and **时钟周期 / Clock Cycles** (this rule). For **clockCycle**: raw = absolute clock cycles from mapped `PipeUtilization.csv` `*_total_cycles` (parallel rename of the former `*_time(us)` column map in [gutter-metrics.spec.md](../../../../specs/core/gutter-metrics.spec.md)); **not** `*_time(us)`, **not** wall-time→cycles conversion, **not** event PMU `pmu_info['total cycle']`. Multi-`block_id` aggregation stays mean of non-`NA` cells until Product picks mean vs sum ([DATA-28](../DATA.md) pattern). Leaf = mapped key raw; **folders / grouping nodes = sum** of child raws. Report-wide \(T=\sum\) leaf raws across **all pipes in all Cards**; **barWidth** = \((\mathrm{raw}/T)\times 100\) (report root = 100%). Labels = bare absolute cycle integers ([UI-46a](UI.md)). `cacheHit` / `task` stay withdrawn. **Fixture gap:** scanned `PipeUtilization.csv` often has only block-level `aic_total_cycles` / `aiv_total_cycles`, not per-pipe `aic_cube_total_cycles` etc. — per-pipe map is the interim target; confirm with Product.
=======
**Interim:** Card-header dropdown has **exactly two** items: **利用率 / Utilization** (unchanged — event coverage per [utilization.spec.md](../../../../specs/core/utilization.spec.md); Product 耗时占比) and **时钟周期 / Clock Cycles** (this rule). For **clockCycle**: raw = absolute clock cycles from mapped `PipeUtilization.csv` `*_total_cycles` (parallel rename of the former `*_time(us)` column map in [gutter-metrics.spec.md](../../../../specs/core/gutter-metrics.spec.md)); **not** `*_time(us)`, **not** wall-time→cycles conversion, **not** event PMU `pmu_info['total cycle']`. Multi-`block_id` aggregation stays mean of non-`NA` cells until Product picks mean vs sum. Leaf = mapped key raw; **folders / grouping nodes = sum** of child raws. Report-wide \(T=\sum\) leaf raws across **all pipes in all Cards**; **barWidth** = \((\mathrm{raw}/T)\times 100\) (report root = 100%). Labels = bare absolute cycle integers ([UI-46a](UI.md)). `cacheHit` / `task` stay withdrawn. **Fixture gap:** scanned `PipeUtilization.csv` often has only block-level `aic_total_cycles` / `aiv_total_cycles`, not per-pipe `aic_cube_total_cycles` etc. Interim **derive** per-pipe cycles as `mean(*_time(us)) × (mean(side_total_cycles)/mean(side_time(us)))` when the direct `*_total_cycles` column is absent — still absolute cycles on the label (no `µs`). Confirm with Product.
>>>>>>> 8f170c8f (feat: show absolute clock cycles on gutter bars)
**Implement / test as:** `gutterMetrics.ts`, `PR-GMET-*`
**Superseded when:** Product confirms source columns, report-wide denom, sum rollup, multi-`block_id` mean vs sum, parallel-pipe oversum OK, and label glyph ([DATA-38](../../questions/DATA.md) / [UI-46](../../questions/UI.md))
