...
- **Source:** Sketch + code audit (2026-09-14). Sketch: the only Peak badge in `src/ui/StatsAside/MemoryTopologyPanel/visual/memory-topology.png` is the L2 pillar plate — the crop the question itself carried (`visual/questions/data-20.png`, now dropped with the question) highlighted exactly that plate, and colour-sampling that highlight box returns the L2 pillar. Code: `peakPct` is produced for `l2` alone. Confirmed against the running app on `sample.lite.rep`: one in-box value (L2 `0.60%`) and ten link magnitudes; no AIC-row label at all (that gap is [DATA-41](../questions/DATA.md)).
