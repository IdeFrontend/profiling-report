...
- **Source:** Product (2026-09-11) confirmed the two-mode selector (时钟周期 + 耗时占比). Product (2026-09-14) confirmed the full formula: absolute mapped `PipeUtilization.csv` `*_total_cycles` labels (per-column derive when missing), shared event-coverage **barWidth**, folder label **sum** (concurrent-pipe oversum accepted), and bare cycle integers ([UI-46](./UI.md)). Supersedes interim [`DATA-38a`](interim/DATA.md).
