...
- **Resolved:** 2026-09-08
- **Was:** OverviewSeries producer (统计分析 Cube / Vector) — which `.npu-rep` embed and fields?
- **Decision:** `Sampling.json` (case-insensitive) Chrome Trace `traceEvents` with `ph: "C"`, `cat: "util"`, numeric `args.value` (utilization **0–100** %). Map counter `name` `CUBE` → `{ id: "cube", label: "Cube" }` and `VECTOR` → `{ id: "vector", label: "Vector" }`. Other util names (`SCALAR`, `MTE*`, `FIXP`, …) stay in the embed but are **not** overview series (no 通信 / third track until a follow-up). Sampling `ts` is **µs** (same family as `PipeTrace.json`); adapter converts to **ns** for `OverviewSeries.points[].t` so scrubbers align with the swimlane. Hide-if-empty remains [DATA-32](./DATA.md) — do **not** invent series from `PipeUtilization`.
- **Specs:** [VIEW_DATA_REQUIREMENTS](../../formats/VIEW_DATA_REQUIREMENTS.md) §3, [VIEW_DATA_MAPPING](../../ui/VIEW_DATA_MAPPING.md) §11.2.7, [view-models](../../../specs/core/view-models.spec.md), [INPUT_FORMATS](../../formats/INPUT_FORMATS.md), [METRICS_AND_TRACE](../../formats/METRICS_AND_TRACE.md)
- **Source:** Product confirmation 2026-09-08 (Sampling.json). Supersedes interim [DATA-32a](interim/DATA.md).
