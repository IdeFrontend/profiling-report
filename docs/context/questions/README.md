...
**Design crops.** The annotated crop is the producer's evidence for a decision, and [../decisions/](../decisions/) entries cite it — but a **resolved** question's crop leaves the open-question store, because [`visual/questions/`](../visual/questions/) is "one image per open question": drop its `manifest.yaml` entry, PNG and `dimensions.json` key in the same change, and let the decision's `Source` line note the dropped path (as [DATA-20](../decisions/DATA.md) does). A **replacement** figure that outlives the question goes to [../visual/](../visual/) instead, next to the decisions that cite it.
