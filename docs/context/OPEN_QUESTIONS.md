...
**Right-panel field / UX ledger:** [HQ_OPEN_QUESTIONS.md](HQ_OPEN_QUESTIONS.md) (HQ DATA 1–29, HQ UI/UX 30–36). Those HQ numbers are not the Q1–Q23 ids in the tables below.
...
- Exact summary tile formulas (Q6) — measured I/O BW confirmed; peak/score still guessed; compute / avg util shown as **N/A** placeholders until formulas land
...
- Context menu: zoom-history scope — 撤销缩放 / 重置缩放 (Q24), Offset contract (Q25)
...
| **Q24** | Zoom-history scope: 撤销缩放 / 重置缩放 | **Open** | Sketch `v930/task-context-menu` rows 2–3 are **撤销缩放 (2)** (Undo zoom, Ctrl+Z, badge = undo depth) and disabled **重置缩放** (Reset zoom) — earlier reading as 翻屏播放/置顶显示 was a misread of the crop. Is zoom-history undo in this library? What does 重置缩放 reset to (initial fit? last file-open state?) and when is it enabled? Ctrl+Z **is** the zoom undo, so no undo collision. Until answered: **omit both from MVP context menu**, no Ctrl+Z binding. Specs when answered: [ContextMenu](../../src/ui/ContextMenu/ContextMenu.spec.md), INTERACTIONS, UX_SPEC. |
...
| **Q26** | 置顶显示 vs Pin row distinction | **Resolved** | **Pin row is the single pinning action.** The sketch has no 置顶显示 item (crop row 3 is disabled 重置缩放 — misread); there is no second, event-scoped pin. Row pin (emit, Ctrl+P, ACs, `pinnedLaneIds`) is deferred to its own PR and leaves the ContextMenu MVP. → [ContextMenu](../../src/ui/ContextMenu/ContextMenu.spec.md) |
...
| Q26 | 2026-08-27 | Pin row is the single pinning action; no 置顶显示 (crop misread); row pin deferred to its own PR | ContextMenu |
