...
| **I-Q6a** | Summary tiles | Show **confirmed** duration from `OpBasicInfo.csv` `Task Duration(us)`. When duration is present, **算力情况** → **I-Q6h** when `computeCard` exists; else title + `N/A`. Avg core util stays **title + `N/A`** until Product defines it. Omit compute/util placeholders when the summary is BW-only so the grid stays rectangular. Duration uses sketch card chrome (bar I-Q6e). Op type is **not** a separate card. I/O BW → **I-Q6g** | Thin duration card + placeholders in `StatsAside` | Data/format spec defines avg-util formula (Q6) |
