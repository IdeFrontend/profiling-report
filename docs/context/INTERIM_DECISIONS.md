...
| **I-Q6g** | I/O bandwidth cards | **Measured:** mean of non-`NA` `aic_main_mem_{read\|write}_bw(GB/s)` / `aiv_*` on `Memory.csv` (also accept headers without `(GB/s)`). **Peak:** max of those same columns (shared across all four slots; same pool as I-Q11d). Fallback **1600 GB/s** only if a side exists but max is empty. **Score:** `round(measured/peak×100)` clamped 0–100. **Bar:** fill = score % of track. **Display:** TB/s = GB/s÷1000, magnitude rounding. **Layout:** same card chrome as duration; aic \| aiv columns (`summary-cards.png`); cards stack full-width (aside ~360px). **NA side:** omit that aic/aiv column; omit card if both NA. `Report.csv` unused (no schema). | `bandwidthCards`, `PR-VM-013`, `PR-STATS-024` | Product peak source (`HardwareInfo` / `Report.csv`?), score formula vs sketch 81, aggregation, `Report.csv` |
