...
| Supersede | When Product or the format/data spec answers the linked Q*: write the decision into owning specs, **remove** the row from [OPEN_QUESTIONS](OPEN_QUESTIONS.md), tag **ANSWERED** on [HQ_OPEN_QUESTIONS](HQ_OPEN_QUESTIONS.md) (keep the block), delete or strike this interim row, and scrub “until Q*” wording — all in the **same change**. See [DEVELOPMENT.md § Resolving open questions](../process/DEVELOPMENT.md#resolving-open-questions). |
...
Canonical Product answers live in the owning **specs** after resolution (see [DEVELOPMENT.md § Resolving open questions](../process/DEVELOPMENT.md#resolving-open-questions)). [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md) holds unanswered / interim rows only; [HQ_OPEN_QUESTIONS.md](HQ_OPEN_QUESTIONS.md) keeps ANSWERED / OBSOLETE notes. Packaging proposals that Product has not confirmed: [PACKAGING_SUGGESTIONS.md](PACKAGING_SUGGESTIONS.md) (also interim until accepted).
...
| **I-Q6a** | Summary tiles | Show **confirmed** duration from `OpBasicInfo.csv` `Task Duration(us)`. Sketch **2×2** (`summary-cards.png` v930 refresh): **整体耗时** \| **AICore 并行使用率**; **算力情况** \| **带宽利用率**. When duration is present, **算力情况** → **I-Q6h** when `computeCard` exists; else title + `N/A`. **AICore 并行使用率** stays **title + `N/A`** until Product defines 并行使用率 / 负载均衡度 (**obsolete:** former 平均核利用率 / 启用 n/m). Omit duration-gated placeholders when the summary is BW-only. Duration chrome → I-Q6e. Op type is **not** a separate card. Bandwidth → **I-Q6g** | Thin duration + placeholders in `StatsAside` | Product AICore-parallel formulas (HQ 9–10) |
...
| **I-Q6g** | Bandwidth card | **Sketch (v930 refresh):** one **带宽利用率** card with **读 \| 写** columns (score **with** `%`, bar, `measured / peak` subtitle). **Measured (confirmed columns):** mean of non-`NA` `aic_main_mem_{read\|write}_bw(GB/s)` / `aiv_*` on `Memory.csv` (first matching header; also accept headers without `(GB/s)`). UI collapses each input/output card’s aic\|aiv sides to a **mean** for 读/写 (aggregation still OPEN for Product). **Peak (still guess):** **1600 GB/s** — **not** max of measured. **Score (still guess):** `round(measured/peak×100)` clamped 0–100. **Display:** **GB/s** (HQ 34; sketch still shows TB/s). `Report.csv` unused. | `bandwidthCards`, `PR-VM-013`, `PR-STATS-024` | Product peak, 读/写 aggregation, score vs sketch 81 |
| **I-Q6h** | 算力情况 card | **Measured (HQ 2, interim):** mean `ArithmeticUtilization.csv` `aic_cube_fops`/`aiv_vec_fops` ÷ mean `aic_time(us)`/`aiv_time(us)` → TFLOPS (`/ 1e6`). **Peak (HQ 3 partial):** cube `16×sizeof(dtype)×16×core×freq×2/1000`; vector `128×core×freq×2/1000` with `ai_*_count` from `HardwareInfo.jsonl`, freq from jsonl `ai_core_frequency_MHZ` or OpBasicInfo `Rated Freq`/`Current Freq`, `sizeof(dtype)` = **2** (FP16) until dtype in CSV. **Score (HQ 4):** `round(measured/peak×100)` clamped 0–100. **Layout (Q33 / sketch):** one card, inner **Cube \| Vector** columns (adapter sides `aic`/`aiv`). Omit a side without both measured and peak; omit card when no sides; else I-Q6a `N/A` placeholder. | `computeCard`, `PR-VM-015`, `PR-STATS-032` | Product MFU formulas (M×K×N), fixed chip peaks, dtype source |
...
6. Summary **2×2** (duration; AICore parallel **N/A**; compute I-Q6h; bandwidth I-Q6g)
