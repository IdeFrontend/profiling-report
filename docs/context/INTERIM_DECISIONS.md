...
| **I-Q6g** | I/O bandwidth cards | **Measured:** mean of non-`NA` `aic_main_mem_{read\|write}_bw(GB/s)` / `aiv_*` on `Memory.csv` (first matching header only; also accept headers without `(GB/s)`). **Peak:** **1600 GB/s** (sketch 1.6 TB/s) for all four aic/aiv × in/out slots — documented HW guess, **not** max of measured columns. **Score:** `round(measured/peak×100)` clamped 0–100 (sketch dummy 81 ≠ ~1% on `out.rep`; follow the ratio). **Bar:** fill = score % of track. **Display:** TB/s = GB/s÷1000, magnitude rounding. **Layout:** same card chrome as duration; aic \| aiv columns (`summary-cards.png`); cards stack full-width (aside ~360px). **NA side:** omit that aic/aiv column; omit card if both NA. `Report.csv` unused (no schema). | `bandwidthCards`, `PR-VM-013`, `PR-STATS-024` | Product peak source (`HardwareInfo` / `Report.csv`?), score formula vs sketch 81, aggregation, `Report.csv` |
