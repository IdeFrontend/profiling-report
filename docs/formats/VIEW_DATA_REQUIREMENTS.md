...
| Bandwidth utilization tile | `Memory.csv` / `summary.jsonl` Memory + `aicore_gm_bw_theoretical(GB/s)` | Sketch **带宽利用率** **读 \| 写**. Display **GB/s** (UI-34). Peak SOL **1600 GB/s** ([DATA-5](../context/decisions/DATA.md)–[DATA-7](../context/decisions/DATA.md)); aic/aiv → 读/写 mean until Product defines aggregation. **Not** `Report.csv` ([DATA-8](../context/decisions/DATA.md)) |
