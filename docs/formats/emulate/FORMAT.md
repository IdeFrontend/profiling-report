...
| `PipeTrace.json` | json | **Recommended** for timeline. Chrome Trace Event format; **µs** `ts`/`dur` ([DATA-41](../../context/decisions/DATA.md)). Native npu_emulate `core_*_tracing_report_*.json` is accepted when `PipeTrace.json` is absent (same µs rule; ignore misleading `displayTimeUnit: "ns"`). **Absence of any trace → null swimlane**, leaf still opens |
