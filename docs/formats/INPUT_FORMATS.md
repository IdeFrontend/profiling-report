...
Common entrypoint for profiling-report **on-disk inputs**. This hub covers the shared **`.npu-rep` container**, how to detect **payload profiles**, and links to profile-specific schemas. It does **not** define hardware metric CSV columns or simulator contract tables.
...
Related UI: [VIEW_DATA_MAPPING.md](../ui/VIEW_DATA_MAPPING.md). Decisions: [PROC-6](../context/decisions/PROC.md) … [PROC-8](../context/decisions/PROC.md), [DATA-40](../context/decisions/DATA.md), [DATA-41](../context/decisions/DATA.md).
...
**No silent remap** ([DATA-40](../context/decisions/DATA.md)): do not invent hardware-shaped metric CSVs from simulator tables. Map each profile into shared `SwimlaneModel` + `ReportViewModel` + `capabilities[]`.
...
| Simulator `PipeTrace.json` | Must be packed in **µs** — producer converts ticks → µs ([DATA-41](../context/decisions/DATA.md)) |
...
| KernelInfo → summary field map | [DATA-42](../context/questions/DATA.md) |
