...
WebGL2 interval backend with sudu-style analytical horizontal coverage AA × SDF round-rect (source-over premul). Same uniform lane fill and 1px dividers as Canvas. Used by default from `SwimlaneCanvas` with a Canvas2D overlay for labels/selection; falls back to `CanvasSwimlaneRenderer` when WebGL2 is unavailable. The mouse-follow cursor bar is a DOM overlay in `SwimlaneView` (under Card strips), not stroked by the renderers.
