...
|---|------|--------|-------------------------|
| 1 | **Deps (library)** — interim DATA-36 encoding + synthetic fixture | **Done** | `chromeTraceToSwimlane.ts` (`args.dependencies`), `playground/depsFixture.ts`, `specs/core/dependencies.spec.md` |
...
| 5 | **Roofline** — interim I-DATA-37 point/ceiling mapping | **Done** | `adaptRep.rooflineFromCsv`, `RooflinePanel.vue`; scaffold [#9](https://github.com/IdeFrontend/profiling-report/pull/9), wiring #16/#21 |
...
| Interim **DATA-36 deps encoding** + synthetic playground fixture | `playground/depsFixture.ts`, adapter in #15 | Anatoly Nikitin / #15 | — |
...
| **Roofline panel** — interim I-DATA-37 in `adaptRep.rooflineFromCsv`, log-log chart | [#9](https://github.com/IdeFrontend/profiling-report/pull/9); wiring #16/#21 | Mikhail Protasov | 2026-08-10–21 |
| Stacked **报告统计** aside (duration → roofline → PIPE → topology) | #16 | Mikhail Protasov | 2026-08-14 |
| I/O bandwidth cards (DATA-33g, adjacent polish) | [#18](https://github.com/IdeFrontend/profiling-report/pull/18) | Mikhail Protasov | 2026-08-19 |
...
| PR | Author | Topic |
|----|--------|-------|
| #25 | Mikhail Protasov | Open questions split (DATA vs UI/UX) |
...
| [#30](https://github.com/IdeFrontend/profiling-report/pull/30) — open-question mockup links + annotated crops | Anatoly Nikitin | Product/docs support; not blocking code exit | Open |
...
| Measure mode: toolbar → drag → band + Δt; no aside sync (UI-42) | **Done** |
| Specs + tests for DATA-36, topology, roofline, measure | **Done** |
...
1. **DATA-36 still open** — `out.rep` has zero real dependency edges; CI and playground demo use `playground/depsFixture.ts` only.
2. **DATA-37 interim** — roofline uses fallback peak constants (`ROOFLINE_PEAK_*`); Product-final formulas deferred to M3.
3. **UI-42 blocked** — measure overlay is local only; aside panels do not recompute for the measured range.
...
| **Mikhail Protasov** | mprotasov | Memory topology + stacked aside, roofline wiring, bandwidth/PIPE polish, open-question docs |
...
| `playground/depsFixture.ts` | Fixture — synthetic DATA-36 dependency chain |

### Specs

| File | Type |
|------|------|
| `specs/core/dependencies.spec.md` | Spec — interim DATA-36 encoding |
