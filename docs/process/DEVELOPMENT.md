...
The open list is for **unanswered** items only: [OPEN_QUESTIONS.md](../context/OPEN_QUESTIONS.md) (status `open` / `partial` / `interim` / `deferred`). Product-final answers live in [DECISIONS.md](../context/DECISIONS.md).
...
2. **File the decision** in [DECISIONS.md](../context/DECISIONS.md) — same bare id as the question, with `Was`, date, decision, specs, source — and **remove** the row from [OPEN_QUESTIONS.md](../context/OPEN_QUESTIONS.md) in the **same change**. For open-question crops, drop the matching `docs/context/visual/questions` manifest entry and PNG. IDs are **permanent** — retire with a `WITHDRAWN` / `DEFERRED` marker, never delete or renumber.
