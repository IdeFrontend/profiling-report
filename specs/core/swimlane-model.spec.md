...
**Ordering.** Processes and threads ordered by first event start time. Within each thread, events sorted by `startTime` ascending. Processes/threads with no events are excluded. Equal-`startTime` ties are broken by longest `duration` first when the viewer assigns multi-row sub-rows (`PR-RENDER-042`); the Chrome Trace converter itself does not re-break ties beyond the ascending `startTime` sort (child-first write order is preserved among equal starts — see `PR-SWIM-009`).
...
## Changelog
- **2026-09-08** — Document equal-`startTime` longest-duration tie-break for multi-row layout (`PR-RENDER-042`); note `sample.lite.rep` as a demo-only exclusivity exception.
- **2026-09-03** — `rebuildLayout` keeps `startTime` order in `eventsByLane`; multi-row layout (not a duration-desc paint sort) handles Chrome-trace nests.
