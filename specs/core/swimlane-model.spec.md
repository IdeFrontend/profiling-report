...
1. **PR-SWIM-011**: Recycled flow ids and the same id in two processes each keep their pairs.
...
- s/f with no matching X interval, missing id/ts, or finish before start → ignored. Nested overlapping X (call stack): innermost containing interval, not the enclosing parent. Flow in idle time between nested slices binds the enclosing slice. Both endpoints in the same X event → no edge. Recycled `id` after a flow closes → one edge per s/f pair, not last-wins.
