...
**Ordering.** Processes and threads ordered by first event start time. Within each thread, events are sorted by `startTime` ascending, with equal-`startTime` ties broken by longest `duration` first (enclosing parent before nested child). `chromeTraceToSwimlane` and multi-row layout (`assignEventRows` / `PR-RENDER-042`) use that same comparator — so viewer sub-rows reproduce the nesting the converter already derived for `nestParents` / `PR-SWIM-009`. `rebuildLayout` does **not** re-sort leaf events: it trusts this order for `eventsByLane` / WebGL gap math and throws if a leaf's events are out of order (adapters always sort; host-built `swimlaneModel` props must too). Processes/threads with no events are excluded.

**Intra-lane exclusivity (producer contract).** Within one leaf lane, event horizontal spans are mutually exclusive: no event nests inside or intersects another, and touching endpoints (`end === next.start`) are siblings. This is the producer guarantee for real `.rep` / CTEF input and a precondition of the WebGL additive interval fill ([`swimlane-renderer.spec.md`](swimlane-renderer.spec.md)). **Demo exception:** `data/sample.lite.rep` (built by `data/build_sample_rep.py`, the default `?fixture=sample`) deliberately nests call-stack windows on op1 SCALAR / MTE2 / ALL so multiline lanes are demosable — not a regression fixture for exclusivity. Standalone Chrome-trace JSON (`chromeTraceToSwimlane`) also does **not** enforce exclusivity — call-stack X events nest on one thread. The viewer restores exclusivity at layout time by splitting overlapping leaf events into non-overlapping sub-rows (multi-height lanes; see the renderer spec "Multi-row lanes"), so the additive fill and hit-testing stay correct by construction rather than degrading.
...
1. **PR-SWIM-009**: Nested overlapping X intervals bind the innermost containing event, including equal-`startTime` pairs (converter sorts longest-duration / enclosing-parent first; Chrome may write the shorter child first in the file).
...
- s/f with no matching X interval, missing id/ts, or finish before start → ignored. Nested overlapping X (call stack): innermost containing interval, not the enclosing parent — including equal-`startTime` parent/child (Chrome may write the shorter child first; the converter reorders longest-duration first before nesting). Touching slices that only share an endpoint are siblings (not nested). Flow in idle time between nested slices binds the enclosing slice. Both endpoints in the same X event → no edge. Distinct overlapping X slices (long parent, shorter worker) stay linked; `pred.end` may exceed `succ.start`. Recycled `id` after a flow closes → one edge per s/f pair, not last-wins. Finish written before its start in the file still pairs when `s.ts <= f.ts`.
