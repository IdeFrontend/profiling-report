...
**Event labels.** When the on-screen (clipped) event width is wide enough (>40 CSS px before scale, or equivalent device width), the title is drawn centered in the visible event rect. Canvas fallback and the WebGL overlay share this layout (device-pixel glyph placement). Vertical centering is optical: the label's ink midpoint (from `actualBoundingBoxAscent`/`Descent`) is centered via an `alphabetic` baseline shifted by `(ascent − descent)/2`, falling back to `middle` when ink metrics are unavailable. Fit policy against the visible rect's available width (measured at the label font size), with **no vertical shrink**: draw as-is when the measured text fits; when the rect is ≥ 80% of the measured width, shrink **horizontally** (`scaleX = rect/measured`, scale around the glyph center); from 30% to 80%, truncate with a trailing `...` (longest fitting prefix, binary-searched); below 30%, skip the label entirely.

**ClearType event labels.** When WebGL2 is the active backend and the browser supports an opaque 2D raster target (`OffscreenCanvas` + `getContext('2d', { alpha: false })`), the WebGL renderer rasterizes each label white-on-black and uploads it preserving raw subpixel RGB (`UNPACK_PREMULTIPLY_ALPHA_WEBGL` false, `UNPACK_COLORSPACE_CONVERSION_WEBGL` none), then re-colorizes via `TEXT_CLEARTYPE_FS`, which mixes the event fill color toward white per subpixel channel at gamma `2.25` and bakes the label background into an opaque quad (alpha = 1) — the baked backdrop uses the same additive formula as the fill (`bg + rgb·dim`, where `bg` is the resting lane fill or `LANE_HOVER_FILL` when that event's row is hovered) so the opaque quad is invisible against the event rect; the overlay skips **normal** event labels in that mode, but still draws the label over a lifted hover/selected state fill (its opaque fill covers the ClearType quad), so non-resting blocks keep a label with the correct contrast. Otherwise (Canvas fallback, or WebGL without an opaque 2D atlas) labels render grayscale via Canvas2D. Label gamma constants: ClearType `2.25`, grayscale `0.625`. Labels use `12px` regular weight (`400`).

**Search / selection emphasis.** Non-matching search hits dim to 25% opacity; when an event is selected, events that are not the selection and not its laid-out neighbors in the active `dependencyMode` and `dependencyDepth` multiply by 0.45 (combined when both apply). Dep neighbors in that filter keep full fill and label brightness; the hovered block also keeps full brightness even when it is neither the selection nor a neighbor — otherwise a light hover fill with a dark label washed by 0.45 is unreadable. Only the clicked event gets the white selection stroke (**2 CSS px** → `round(2 × dpr)` device px). Canvas uses `globalAlpha`; WebGL rebuilds per-dim mesh layers and passes premul `uColor` RGB×dim with alpha=dim (hover exemption is applied in the overlay pass that paints the hover fill). Labels use the same dim (overlay + Canvas fallback); search non-matches omit labels. Clearing search and selection restores full opacity.

**Event fills.** Each lane has one base colour, and the three interaction states are derived from it in OKLCH (`src/domain/oklch.ts`) rather than hand-picked: `normal` = base, `hover` = `L + 0.33`, `selected` = `L + 0.33` with `C × 1.05`. Experiment: hover shares selection's lightness so the lift is unmissable; the white ring and chroma bump alone mark selection. `L` is perceptual, so one step reads the same on every lane. Requests outside sRGB — routine for the oranges at this lift — keep `L` and hue and give up chroma. A block that is both selected and hovered paints `selected` (`eventStateOf`), and the white selection ring paints over whichever fill results.
...
1. **PR-RENDER-010**: `eventEmphasisDim` matches Canvas factors (search 0.25 × selection 0.45); the hovered block stays at full strength under a selection; WebGL setSelection rebuilds emphasis layers and render does not throw.
...
1. **PR-RENDER-023**: Text shaders export sudu gamma constants (`CLEARTYPE_TEXT_POW` 2.25, `GRAYSCALE_TEXT_POW` 0.625); `eventLabelFont` uses the shared CSS px size; `clearTypeRasterSupported` is false without an opaque 2D context; `fitTextWidth` truncates an over-wide label with a trailing `...` (longest fitting prefix) and strips a trailing space/`_` from the cut text; `fitEventLabel` picks draw / horizontal-shrink / truncate / skip from the measured width vs the available width (≥80% shrinks, 30–80% truncates, <30% skips).
...
1. **PR-RENDER-025**: ClearType label backdrop uses the hovered row's `LANE_HOVER_FILL` when that event's lane is hovered, so the label rect matches the event rect.
