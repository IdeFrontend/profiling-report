...
1. **PR-RENDER-025**: Summary bars render a gray fill with a dimmed `SUMMARY_LABEL_COLOR` "N tasks" label (normal font) and lift to the OKLCH hover fill under the pointer; they are never dimmed by search/selection and never selected/ringed.
...
- **2026-09-03** — Summary bars become interactive: hover lift, hit-testable, dimmed `SUMMARY_LABEL_COLOR` "N tasks" label (normal font); PR-RENDER-024 / PR-RENDER-025.
