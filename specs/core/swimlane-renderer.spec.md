...
1. **PR-RENDER-024**: `assignEventRows` / `leafRowCount` place overlapping leaf events into non-overlapping sub-rows by greedy first-fit on `startTime` (tie-break longest `duration` first); a leaf with no overlaps has one sub-row and touching events stay siblings on the same sub-row.
1. **PR-RENDER-025**: `FlatLane.rowCount` sizes a leaf at `rowCount × LANE_HEIGHT` in `rebuildLayout`, `contentHeightFromLayout`/`contentHeightFromModel`, and `layoutHeaders`; folders and Card headers stay `LANE_HEIGHT` / `LANE_GROUP_HEADER_HEIGHT`.
1. **PR-RENDER-026**: Each event block draws vertically centered in its own sub-row (`item.y` = sub-row top); the Canvas lane fill, hover tint, and bottom divider span `rowCount × LANE_HEIGHT` with no internal dividers.
1. **PR-RENDER-027**: `hitTestLayout` resolves an event in the correct sub-row (a pointer over sub-row N never hits an event in another sub-row).
1. **PR-RENDER-028**: WebGL builds one interval mesh per (lane, sub-row) and draws each sub-row at its own Y, so each sub-row is mutually exclusive and the additive fill never saturates an overlap.
...
## Changelog
- **2026-09-03** — Multi-row lanes: overlapping leaf events split into non-overlapping sub-rows (greedy first-fit), leaf height `rowCount × LANE_HEIGHT`, hit-test/mesh per sub-row; restores the additive-fill invariant for overlapping standalone Chrome-trace lanes. (PR-RENDER-024–028)
