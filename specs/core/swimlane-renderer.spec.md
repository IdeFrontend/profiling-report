...
1. **PR-RENDER-013**: Selected event's predecessors/successors keep their original fill and label color; non-neighbors render solid gray `#4D4D4D`.
...
1. **PR-RENDER-023**: Selecting an event paints non-selected/non-neighbor blocks solid gray `#4D4D4D` with label `#969696`, keeps the selected block's lifted state fill and its contrast label, and draws no white selection ring.
