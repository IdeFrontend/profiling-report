...
1. **PR-RENDER-025**: Summary bars render a gray fill with no label (task count is tooltip-only) and lift to the OKLCH hover fill under the pointer; they are never dimmed by search/selection and never selected/ringed.
...
- **2026-09-03** — Summary bars become interactive: hover lift, hit-testable; task count is tooltip-only (no on-bar label); PR-RENDER-024 / PR-RENDER-025.
