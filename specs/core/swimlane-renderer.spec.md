...
- Corner decision uses the **CSS-px** raw width at any dpr: WebGL compares device width against the `uRR.z` threshold (`rrSwitchThreshold × dpr`, rounded), so the `rawW < 4 CSS px` cutoff is density-independent.
...
## Changelog
- **2026-08-31** — Corner radius is now a CSS-pixel policy (`minRR`/`maxRR`/`rrSwitchThreshold` in `shaders.ts`): 1 CSS px when raw CSS width < 4, else 2 CSS px, then × dpr, rounded `((x+0.5)|0)`. WebGL uploads the device-px trio as one `vec3` `uRR`; Canvas `eventRadius(cssW, dpr)` uses the same constants. (PR-RENDER-017)
