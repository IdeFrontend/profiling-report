...
1. **PR-RENDER-023**: Text shaders export sudu gamma constants (`CLEARTYPE_TEXT_POW` 2.25, `GRAYSCALE_TEXT_POW` 0.625); `eventLabelFont` uses the shared CSS px size; `clearTypeRasterSupported` is false without an opaque 2D context; `fitTextWidth` truncates an over-wide label with a trailing `...` (longest fitting prefix).
