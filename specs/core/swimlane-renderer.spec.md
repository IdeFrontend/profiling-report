...
1. **PR-RENDER-024**: `eventGaps` returns a lane event's `[gapPrev, gapNext]` from whole-lane neighbors — `EDGE_GAP` on the true lane boundaries, real neighbors across chunk splits in both directions — and `setVbSquareWithGaps` writes one 6-float/vertex quad carrying both gaps per vertex.
...
## Changelog
- **2026-09-04** — Extracted `eventGaps` / `EDGE_GAP` and exported `setVbSquareWithGaps` so the isolated-event gap math and vertex packing are unit-testable without a GL context; `createChunksFromPairs` computes gaps per event (global index) and materializes only one chunk's gaps at a time (PR-RENDER-024).
