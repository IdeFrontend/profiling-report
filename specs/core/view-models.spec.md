...
**Overview series.** From product `Sampling.json` Chrome Trace `ph:"C"` counters ([DATA-39a](../context/decisions/interim/DATA.md)): one `OverviewSeries` per distinct counter `name` (`id`/`label` = `name`); `points[{t,v}]` from `ts` (µs→canonical ns) and finite `args.value`. Empty when Sampling absent or no counters — hide UI ([DATA-32](../context/decisions/DATA.md)). Do not invent from `PipeUtilization`.
...
3. **PR-VM-003** — Overview series from `Sampling.json` `ph:C` (DATA-39a): one track per counter name on product fixtures with Sampling; empty when Sampling absent (`out.rep`); never invented from PipeUtilization.
...
DATA-33, DATA-33b, DATA-33c, DATA-33d, DATA-33f, DATA-39a, DATA-34a, DATA-37a–f. [rep-format](./rep-format.spec.md), [swimlane-model](./swimlane-model.spec.md).
