...
11. **PR-VM-012** — Topology labels come only from the requested `block_id`. The adapter snapshot for a product pack is the `summary.jsonl` `All` aggregate; `firstLabelledMemoryTopology` is used **only** for a classic `.rep` with no `summary.jsonl` (see PR-VM-017).
