...
**Block scope (DATA-19 / DATA-28 / DATA-29).** One selector scopes every CSV-backed widget: `All` reads the `summary.jsonl` category mean (the producer's own non-`NA` aggregate across `block_id`), a picked id reads that block's CSV row. Applies to PIPE (`PipeUtilization`), the memory topology (`Memory` / `MemoryL0` / `MemoryUB` / `L2Cache`), the BW cards (`Memory.csv` row sides summed per DATA-8), the compute card (`ArithmeticUtilization.csv` row measured, chip-level peak) and the roofline (`ArithmeticUtilization.csv` + `Memory.csv` rows). `HardwareInfo` and `OpInfoSummary`-only metrics (AI Core 并行使用率 / 负载均衡度) have no per-block source and stay op-level. When `summary.jsonl` is absent the `All` aggregate does not exist, so PIPE and the memory diagram fall back to the CSV data (the classic `.rep` path).
...
18. **PR-VM-019** — `computeCardFromRows(rows, summary)` measures the block's `ArithmeticUtilization.csv` row and takes the chip-level theoretical FLOPS from `OpInfoSummary`; no side without both measured and peak (DATA-19).
