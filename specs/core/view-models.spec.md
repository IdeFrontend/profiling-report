...
**Emulate profile.** When adapting an emulate leaf ([adapt-emulate](./adapt-emulate.spec.md)), compute CSV mappings above do not apply. Fill swimlane from `PipeTrace.json` (µs); thin summary from KernelInfo/summary (interim DATA-47a); PIPE from `PipesUtilization` / hist when present; omit memory topology / roofline / overview until emulate mappers exist ([DATA-45](../../docs/context/decisions/DATA.md)). Gap matrix: [VIEW_DATA_REQUIREMENTS](../../docs/formats/VIEW_DATA_REQUIREMENTS.md).
...
## Changelog
- **2026-09-14** — Note emulate profile adaptation; hardware CSV mappings unchanged ([DATA-45](../../docs/context/decisions/DATA.md)).
