...
**Swimlane.** When `PipeTrace.json` **or** a native `*_tracing_report_*.json` is present, build `SwimlaneModel` via `chromeTraceToSwimlane` with `sourceTimeUnit: 'us'` ([DATA-41](../../docs/context/decisions/DATA.md)). When absent, `swimlaneModel` is **null** (open still succeeds). Corrupt Trace JSON → throw.
