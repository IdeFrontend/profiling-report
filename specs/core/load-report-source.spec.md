...
1. **`npu-rep`** — product container ([PROC-2](../../docs/context/decisions/PROC.md)). Routes by `fileInfoLength` to the 160-byte product layout or the interim 164-byte sample layout ([npu-rep.spec](./npu-rep.spec.md)), then adapts nested or flat operators → full `AdaptedReport`. Per leaf: if emulate `manifest.json` (thin profile or export catalog), use **adaptEmulate**; else **adaptPayloads** (hardware) ([PROC-8](../../docs/context/decisions/PROC.md), [ADAPTERS.md](../../docs/formats/ADAPTERS.md)).
...
1. **PR-JSON-004**: A `npu-rep` leaf with emulate `manifest.json` (thin profile or export catalog) is routed to the emulate adapter path (not required to contain compute metric CSVs; PipeTrace optional).
...
## Changelog
- **2026-09-14** — PR-JSON-004: route simulator leaves by `manifest.json` ([PROC-8](../../docs/context/decisions/PROC.md)).
