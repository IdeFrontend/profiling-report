...
1. **PR-JSON-004**: A `npu-rep` leaf with `EmulateManifest.json` (`profile: "emulate"`) is routed to the emulate adapter path (not required to contain compute metric CSVs).
...
## Changelog
- **2026-09-14** — PR-JSON-004: route simulator leaves by `EmulateManifest.json` ([PROC-8](../../docs/context/decisions/PROC.md)).
