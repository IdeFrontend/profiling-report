...
1. **PR-JSON-004**: A `npu-rep` leaf with `SimulatorManifest.json` (`profile: "simulator"`) is routed to the simulator adapter path (not required to contain hardware metric CSVs).
...
## Changelog
- **2026-09-14** — PR-JSON-004: route simulator leaves by `SimulatorManifest.json` ([PROC-8](../../docs/context/decisions/PROC.md)).
