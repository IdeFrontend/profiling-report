...
**Two-tier auto scale** ([UI-40](../../docs/context/decisions/UI.md)). Wall-time labels use `TimeScaleUnit` (`s` / `ms` / `us` / `ns`).
...
[UI-40](../../docs/context/decisions/UI.md) — Time (auto) vs CPU clocks; [UI-45](../../docs/context/decisions/UI.md) — derived cycles from OpBasicInfo (`currentFreq`, else `ratedFreq`).

## Changelog
- **2026-09-10** — Product confirmed UI-40 / UI-45 (derived cycles); drop interim UI-40a / open UI-45 wording.
...
- **2026-09-02** — PR-TIME-010 cycles mode (UI-40a derived cycles); `opts.mode` / `opts.clockFreqMHz` on all formatters.
