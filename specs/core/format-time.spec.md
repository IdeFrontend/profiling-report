...
`opts = { significantDigits?, mode?, clockFreqMHz?, totalSpanNs? }` — `mode: 'cycles'` renders CPU clocks instead of wall time.
...
**Cycles mode (I-Q14).** When `mode: 'cycles'` and a valid `clockFreqMHz`, every formatter renders derived CPU clocks `cycles = ns × freqMHz / 1000`, rounded to an integer, as a **number only** — no `cyc` / `cycles` suffix. The value is space-grouped in 3-digit groups and **zero-padded to a fixed width** shared by every cycles label: the group count is derived from the whole trace's cycle total (`nsToCycles(totalSpanNs, freqMHz)`), so the axis reads as a fixed-width counter (`010 325`, `000 000`). Missing / invalid `clockFreqMHz` → `—`; `formatTimeParts*` returns an empty `unit`. `clockFreqMHz` comes from `resolveClockFreqMHz` (`currentFreq ?? ratedFreq` from `OpBasicInfo`, MHz). **Not** per-event `*_total_cycles`; the viewport coarse base + remainder chrome is disabled in cycles. Open: true vs derived — [Q23](../../docs/context/OPEN_QUESTIONS.md) / [HQ 38](../../docs/context/HQ_OPEN_QUESTIONS.md).
...
1. **PR-TIME-010** — cycles conversion, freq resolve, and fixed-width zero-padded cycle formatting (no unit; `—` without freq).
