...
**Profiles.** After payloads are available for a leaf, if `EmulateManifest.json` is present with `"profile": "emulate"`, adapt via the **emulate** path ([emulate-format](./emulate-format.spec.md), [adapt-emulate](./adapt-emulate.spec.md)); otherwise use the **compute** path (`adaptPayloads`). Head `origin` remains `1` until [PROC-9](../../docs/context/questions/PROC.md). Emulate leaves MAY omit compute metric CSVs ([DATA-40](../../docs/context/decisions/DATA.md)).
...
12. **PR-NPU-012** — A leaf whose payloads include `EmulateManifest.json` with `profile: "emulate"` is classified as emulate and MUST NOT require compute `OpBasicInfo.csv` / `PipeUtilization.csv` to load ([PROC-8](../../docs/context/decisions/PROC.md), [DATA-40](../../docs/context/decisions/DATA.md)).
...
## Changelog
- **2026-09-14** — PR-NPU-012: simulator leaf detection via `EmulateManifest.json`; no hardware CSV requirement ([PROC-8](../../docs/context/decisions/PROC.md), [DATA-40](../../docs/context/decisions/DATA.md)).
