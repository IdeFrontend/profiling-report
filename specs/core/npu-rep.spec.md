...
**Profiles.** After payloads are available for a leaf, if `SimulatorManifest.json` is present with `"profile": "simulator"`, adapt via the **simulator** path ([simulator-format](./simulator-format.spec.md), [adapt-simulator](./adapt-simulator.spec.md)); otherwise use the **hardware** path (`adaptPayloads`). Head `origin` remains `1` until [PROC-9](../../docs/context/questions/PROC.md). Simulator leaves MAY omit hardware metric CSVs ([DATA-40](../../docs/context/decisions/DATA.md)).
...
12. **PR-NPU-012** — A leaf whose payloads include `SimulatorManifest.json` with `profile: "simulator"` is classified as simulator and MUST NOT require hardware `OpBasicInfo.csv` / `PipeUtilization.csv` to load ([PROC-8](../../docs/context/decisions/PROC.md), [DATA-40](../../docs/context/decisions/DATA.md)).
...
## Changelog
- **2026-09-14** — PR-NPU-012: simulator leaf detection via `SimulatorManifest.json`; no hardware CSV requirement ([PROC-8](../../docs/context/decisions/PROC.md), [DATA-40](../../docs/context/decisions/DATA.md)).
