...
1. **PR-STRESS-008**: Stress models emit no `bands` (collapsed-group summary bars are computed by the viewer, not the producer).
...
## Changelog
- **2026-09-03** — Stress models no longer emit `bands` (summary bars are viewer-computed); PR-STRESS-008.
