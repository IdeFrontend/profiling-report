...
**Third loading path (backend-extracted).** Host passes `files` (a name→bytes map or `{ name, data }[]` of the backend `npu-compute` extraction) — the library adapts the extracted folder via `loadReportFiles`/`adaptPayloads`. The host runs the backend extraction (explicit `npu-compute` path → autodetect + version check → auto-download) and hands the resulting files to the library; the library never invokes or bundles `npu-compute`.
...
PKG-1 … PKG-3, UI-41 — packaging and integration specifics (see PACKAGING_SUGGESTIONS.md).
