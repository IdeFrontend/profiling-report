...
**Feature flags.** `capabilities` (`ReportCapability[]`) controls which sub-panels can be rendered. By default (undefined/empty), only MVP features are shown. Adding capability strings (`'roofline'`, `'dependencies'`, `'memoryDiagram'`, `'hardwareDetails'`, `'sourceTab'`, `'cacheTab'`, `'aicpu'`) unlocks corresponding Phase 2 panel modes. `'dependencies'`, `'memoryDiagram'` and `'hardwareDetails'` are derived by the adapter from the parsed source; `'roofline'` is **opt-in only** — it is a Phase 2 surface outside the current release, so the adapter never advertises it and a host must pass it explicitly to mount the panel. The array is joined into a data attribute for CSS and test hooking.
