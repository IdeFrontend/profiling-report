...
## Changelog
- **2026-09-25** — `open-performance-hints-in-problems` added to the host-facing events and the `environment` prop documented: the 性能分析 详情 trigger routes to the host Problems view on `'vscode'`, to the internal dock on `'browser'`/omitted ([environments.ts](../../src/ui/environments.ts)).
