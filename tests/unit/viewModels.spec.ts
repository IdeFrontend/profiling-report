...
describe('PR-VM: report view-models (interim)', () => {
  ...
  it('PR-VM-012: topology labels are block-scoped; snapshot is summary.jsonl, first drawable block is the classic-.rep fallback', () => {
    ...
    // PR-VM-012 / PR-VM-019: a product pack snapshots the summary.jsonl aggregate, not that first drawable block.
  ...
  it('PR-VM-019 (DATA-19/28/29): All scope reads summary.jsonl, a picked block reads its CSV row', () => {
    const encoder = new TextEncoder();
    const payloads = {
  ...
  it('PR-VM-019b (DATA-19/29): All roofline reads the summary.jsonl categories, not the CSV mean', () => {
    const encoder = new TextEncoder();
    const payloads = {
      // Producer aggregate: deliberately different from the per-block CSVs below.
      'summary.jsonl': encoder.encode(
  ...
  it('PR-VM-020 (DATA-8/19): bandwidthCardsFromRows sums a block’s aic + aiv sides per direction', () => {
    const rows = [
      {
        block_id: '2',
        'aic_main_mem_read_bw(GB/s)': '80',
  ...
  it('PR-VM-021 (DATA-19): computeCardFromRows measures the block row, peaks stay chip-level', () => {
  ...
  it('PR-VM-022 (DATA-19): rooflineFromRows honours the rows it is given', () => {
